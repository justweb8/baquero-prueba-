...
function imInicializar(){
  ...
  // Drag & drop
  const zona = document.querySelector('#sec-importar .im-drop-zone, #sec-importar .im-zona');
  if(zona && !zona._imConectado){
    zona.addEventListener('dragover', e => { e.preventDefault(); zona.style.borderColor='#2E7DD6'; });
...
async function imCargarArchivo(file){
  ...
  const nombreEl = document.querySelector('#sec-importar .im-file-nombre, #sec-importar .im-nombre-archivo');
  if(nombreEl) nombreEl.textContent = file.name;

  const btnZona  = document.querySelector('#sec-importar .im-drop-zone .im-drop-sub, #sec-importar .im-zona-sub');
  if(btnZona) btnZona.textContent = `📄 ${file.name} (${(file.size/1024).toFixed(1)} KB)`;
...
function imMostrarPreview(rows){
  ...
  const datos   = rows.slice(1,6); // Primeras 5 filas

  prev.innerHTML = `
    <div style="font-family:'Montserrat',sans-serif;font-size:13px;font-weight:700;color:#0D2B6B;margin-bottom:10px;">
      Vista previa — ${rows.length-1} registros · ${headers.length} columnas
    </div>
    <div style="overflow-x:auto;border-radius:8px;border:1px solid #E0E8F4;">
      <table style="width:100%;border-collapse:collapse;font-size:12px;">
        <thead>
          <tr style="background:#F8FAFF;">
            ${headers.map(h=>`<th style="padding:8px 12px;text-align:left;font-weight:700;color:#0D2B6B;white-space:nowrap;border-bottom:1px solid #E0E8F4;">${escH(String(h))}</th>`).join('')}
          </tr>
        </thead>
        <tbody>
          ${datos.map(row=>`<tr style="border-bottom:1px solid #F0F4FA;">
            ${row.map(c=>`<td style="padding:7px 12px;color:#5A6A85;white-space:nowrap;">${escH(String(c||''))}</td>`).join('')}
          </tr>`).join('')}
        </tbody>
      </table>
    </div>
    ${rows.length > 6 ? `<div style="font-size:11px;color:#8FA3BF;margin-top:6px;">... y ${rows.length-6} registros más</div>` : ''}
  `;
...
function imQuitarArchivo(){
  ...
  const btnZona = document.querySelector('#sec-importar .im-drop-zone .im-drop-sub, #sec-importar .im-zona-sub');
  if(btnZona) btnZona.textContent = 'Arrastra tu archivo aquí o haz clic para seleccionar';
