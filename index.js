...
/* ── Variables globales DB — disponibles desde el inicio ── */
var DB_ANIMALES=[], DB_SALUD=[], DB_INSEM=[], DB_PARTOS=[];
var DB_GASTOS=[], DB_INVENTARIO=[], DB_PROVEEDORES=[], DB_BANCO=[], DB_DOCS=[];
var SESSION=null;
...
function abrirSeccion(idSec){
  ...
  if(!document.getElementById(idSec)){
    if(idSec==='sec-buscar') _crearSecBuscar();
    else if(idSec==='sec-arbol') _crearSecArbol();
  }

  const sec = document.getElementById(idSec);
  if(sec){
    sec.style.cssText='display:flex !important;flex-direction:column;flex:1;overflow-y:auto;overflow-x:hidden;min-height:0;';
...
function _crearSecBuscar(){
  const main = document.querySelector('.d-main') || document.querySelector('main') || document.body;
  const div  = document.createElement('div');
  div.id     = 'sec-buscar';
  div.style.cssText = 'display:none;flex-direction:column;overflow-y:auto;overflow-x:hidden;flex:1;padding:0;';
  div.innerHTML = `
    <div style="background:linear-gradient(135deg,#0D2B6B,#1a4a9e);padding:28px;color:#fff;">
      <div style="font-family:'Montserrat',sans-serif;font-size:22px;font-weight:800;margin-bottom:6px;">🔍 Buscar Animal</div>
      <div style="font-size:13px;opacity:.75;margin-bottom:14px;">Encuentra cualquier animal por arete, nombre, raza o características</div>
      <div style="position:relative;">
        <input id="bus-input" type="text" placeholder="Buscar por arete, nombre, raza..." autocomplete="off"
          oninput="buscarAnimal(this.value)"
          style="width:100%;padding:13px 46px 13px 18px;border-radius:12px;border:none;font-size:15px;outline:none;box-sizing:border-box;font-family:'Open Sans',sans-serif;">
        <span style="position:absolute;right:14px;top:50%;transform:translateY(-50%);font-size:18px;">🔍</span>
      </div>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;padding:14px 20px;background:#F8FAFF;border-bottom:1px solid #E8EEF8;">
      <button class="bus-filtro-btn on" onclick="busFiltro(this,'')" style="padding:6px 14px;border-radius:20px;border:1.5px solid #2E7DD6;background:#2E7DD6;color:#fff;font-size:12px;font-weight:600;cursor:pointer;">Todos</button>
      <button class="bus-filtro-btn" onclick="busFiltro(this,'Activo')" style="padding:6px 14px;border-radius:20px;border:1.5px solid #E0E8F4;background:#fff;color:#5A6A85;font-size:12px;font-weight:600;cursor:pointer;">✅ Activos</button>
      <button class="bus-filtro-btn" onclick="busFiltro(this,'Gestante')" style="padding:6px 14px;border-radius:20px;border:1.5px solid #E0E8F4;background:#fff;color:#5A6A85;font-size:12px;font-weight:600;cursor:pointer;">🤰 Gestantes</button>
      <button class="bus-filtro-btn" onclick="busFiltro(this,'En tratamiento')" style="padding:6px 14px;border-radius:20px;border:1.5px solid #E0E8F4;background:#fff;color:#5A6A85;font-size:12px;font-weight:600;cursor:pointer;">💊 En tratamiento</button>
      <button class="bus-filtro-btn" onclick="busFiltro(this,'Hembra')" data-campo="sexo" style="padding:6px 14px;border-radius:20px;border:1.5px solid #E0E8F4;background:#fff;color:#5A6A85;font-size:12px;font-weight:600;cursor:pointer;">♀ Hembras</button>
      <button class="bus-filtro-btn" onclick="busFiltro(this,'Macho')" data-campo="sexo" style="padding:6px 14px;border-radius:20px;border:1.5px solid #E0E8F4;background:#fff;color:#5A6A85;font-size:12px;font-weight:600;cursor:pointer;">♂ Machos</button>
    </div>
    <div style="padding:18px 20px;flex:1;">
      <div style="display:flex;gap:10px;margin-bottom:16px;flex-wrap:wrap;">
        <div style="background:#fff;border:1px solid #E8EEF8;border-radius:12px;padding:10px 18px;flex:1;min-width:100px;text-align:center;">
          <div id="bus-s-total" style="font-family:'Montserrat',sans-serif;font-size:22px;font-weight:800;color:#0D2B6B;">0</div>
          <div style="font-size:11px;color:#8FA3BF;">Total</div>
        </div>
        <div style="background:#fff;border:1px solid #E8EEF8;border-radius:12px;padding:10px 18px;flex:1;min-width:100px;text-align:center;">
          <div id="bus-s-activos" style="font-family:'Montserrat',sans-serif;font-size:22px;font-weight:800;color:#22C55E;">0</div>
          <div style="font-size:11px;color:#8FA3BF;">Activos</div>
        </div>
        <div style="background:#fff;border:1px solid #E8EEF8;border-radius:12px;padding:10px 18px;flex:1;min-width:100px;text-align:center;">
          <div id="bus-s-gestantes" style="font-family:'Montserrat',sans-serif;font-size:22px;font-weight:800;color:#9333EA;">0</div>
          <div style="font-size:11px;color:#8FA3BF;">Gestantes</div>
        </div>
        <div style="background:#fff;border:1px solid #E8EEF8;border-radius:12px;padding:10px 18px;flex:1;min-width:100px;text-align:center;">
          <div id="bus-s-resultados" style="font-family:'Montserrat',sans-serif;font-size:22px;font-weight:800;color:#2E7DD6;">0</div>
          <div style="font-size:11px;color:#8FA3BF;">Resultados</div>
        </div>
      </div>
      <div id="bus-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px;">
        <div style="text-align:center;padding:40px;color:#8FA3BF;grid-column:1/-1;">🐄 Escribe para buscar animales...</div>
      </div>
    </div>`;
  main.appendChild(div);
}

function _crearSecArbol(){
  const main = document.querySelector('.d-main') || document.querySelector('main') || document.body;
  const div  = document.createElement('div');
  div.id     = 'sec-arbol';
  div.style.cssText = 'display:none;flex-direction:column;overflow-y:auto;overflow-x:hidden;flex:1;padding:28px;';
  div.innerHTML = `
    <div style="font-family:'Montserrat',sans-serif;font-size:22px;font-weight:800;color:#0D2B6B;margin-bottom:8px;">🌳 Árbol Genealógico</div>
    <div style="font-size:13px;color:#8FA3BF;margin-bottom:20px;">Visualiza el linaje y parentesco de tus animales</div>
    <div id="arbol-cont" style="background:#fff;border-radius:14px;border:1px solid #E8EEF8;padding:20px;min-height:400px;display:flex;align-items:center;justify-content:center;">
      <div style="text-align:center;color:#8FA3BF;">
        <div style="font-size:48px;margin-bottom:12px;">🌳</div>
        <div>Selecciona un animal para ver su árbol genealógico</div>
      </div>
    </div>`;
  main.appendChild(div);
}
...
function navegar(seccion, el){
  ...
  switch(seccion){
    ...
    case 'suscripcion':
      ...
      break;
    case 'buscar':
      abrirSeccion('sec-buscar');
      break;
    case 'arbol':
      abrirSeccion('sec-arbol');
      break;
    default:
      cerrarSeccion();
...
/* ── MEJORAS DE NAVEGACIÓN — cargar datos reales ── */
(function(){
  var _navOrig = window.navegar || navegar;
  window.navegar = function(seccion, el){
    // Llamar la función original primero
    _navOrig(seccion, el);
    // Luego cargar datos reales
    switch(seccion){
      case 'inicio':      if(typeof cargarInicio==='function')      cargarInicio();      break;
      case 'animales':    if(typeof cargarAnimales==='function')    cargarAnimales();    break;
      case 'salud':       if(typeof cargarSalud==='function')       cargarSalud();       break;
      case 'inseminacion':if(typeof cargarInsem==='function')       cargarInsem();       break;
      case 'partos':      if(typeof cargarPartos==='function')      cargarPartos();      break;
      case 'alertas':     if(typeof renderAlertas==='function')     renderAlertas();     break;
      case 'reportes':    if(typeof renderReportes==='function')    renderReportes();    break;
      case 'historial':   if(typeof cargarHistorial==='function')   cargarHistorial();   break;
      case 'gastos':      if(typeof cargarGastos==='function')      cargarGastos();      break;
      case 'inventario':  if(typeof cargarInventario==='function')  cargarInventario();  break;
      case 'proveedores': if(typeof cargarProveedores==='function') cargarProveedores(); break;
      case 'banco-genetico':if(typeof cargarBanco==='function')     cargarBanco();       break;
...
async function pfGuardarRancho(){
  ...
  try{
    ...
    if(razon)     payload.razon_social = razon;
    ...
    if(!res.ok){
      ...
      // Si falla por columna inexistente, intentar solo con nombre
      if(e.message&&e.message.includes('column')){
        const res2=await fetch(
          `${SB_URL}/rest/v1/ranchos?id=eq.${SESSION.rancho_id}`,
          {method:'PATCH',headers:SB_HEADERS,body:JSON.stringify({nombre})}
        );
        if(!res2.ok){const e2=await res2.json();throw new Error(e2.message||'Error al guardar');}
      } else {
...
function dcRenderTabla(lista){
  ...
  tbody.innerHTML=filtrada.map((d,i)=>{
    const fecha=d.fecha||( d.created_at?new Date(d.created_at).toLocaleDateString('es-PE'):'—');
    const tipo=d.tipo||'Documento';
    const badgeClass={'Ficha Veterinaria':'dc-badge-vet','Guía de Remisión':'dc-badge-rem','Certificado Sanitario':'dc-badge-san'}[tipo]||'dc-badge-gen';
    return `<tr>
      <td style="color:#8FA3BF;font-size:12px;">${d.num_doc||String(i+1).padStart(3,'0')}</td>
      <td>
        <div style="font-weight:600;color:#0D2B6B;font-size:13px;">${escH(d.descripcion||tipo)}</div>
        <div style="font-size:11px;color:#8FA3BF;">${d.hora||''}</div>
      </td>
      <td><span class="${badgeClass}">${escH(tipo)}</span></td>
      <td style="font-size:12px;color:#5A6A85;">${fecha}</td>
      <td><span class="dc-estado-ok">Emitido</span></td>
      <td><div class="dc-tbl-btns">
        <button class="dc-tbl-btn" title="Ver" style="cursor:pointer;">
...
function dcSelTipo(el, tipo){
  document.querySelectorAll('#sec-documentos .dc-tipo, .dc-tipo').forEach(t=>{
    t.style.borderColor=''; t.style.boxShadow=''; t.style.background='';
  });
  el.style.borderColor='#2E7DD6';
  el.style.background='#f0f7ff';
  _dcTipoSel=tipo;
  ...
  setTimeout(()=>{
    if(tipo==='Ficha Veterinaria')     dcAbrirModalFicha();
    else if(tipo==='Guía de Remisión') dcAbrirModalGuia();
    else if(tipo==='Certificado Sanitario') dcAbrirModalCertificado();
  },100);
}
...
/* ══════════════════════════════════════════
   MODALES DE DOCUMENTOS — 3 tipos
   ══════════════════════════════════════════ */

/* ── Helper: poblar select de animales ── */
function _dcSelectAnimales(selId){
  const sel=document.getElementById(selId);
  if(!sel) return;
  sel.innerHTML='<option value="">— Selecciona —</option>'+
    DB_ANIMALES.map(a=>`<option value="${escH(a.arete)}">${escH(a.arete)}${a.nombre?' — '+escH(a.nombre):''} · ${escH(a.raza||'')} · ${a.sexo==='Hembra'?'Hembra':'Macho'} · ${a.peso?a.peso+' kg':'—'}</option>`).join('');
}

/* ── Ficha Veterinaria ── */
function dcAbrirModalFicha(){
  if(!document.getElementById('dc-m-ficha')){
    const d=document.createElement('div');
    d.innerHTML=`
    <div id="dc-m-ficha" style="display:none;position:fixed;inset:0;background:rgba(13,43,107,.5);z-index:9999;align-items:center;justify-content:center;padding:16px;" onclick="if(event.target===this)this.style.display='none'">
      <div style="background:#fff;border-radius:18px;width:100%;max-width:480px;max-height:90vh;overflow-y:auto;padding:24px;box-shadow:0 20px 60px rgba(0,0,0,.2);">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;">
          <div style="font-family:'Montserrat',sans-serif;font-size:17px;font-weight:700;color:#0D2B6B;">🩺 Ficha Veterinaria</div>
          <button onclick="document.getElementById('dc-m-ficha').style.display='none'" style="background:none;border:none;font-size:24px;cursor:pointer;color:#9eaaba;">×</button>
        </div>
        <div style="display:flex;flex-direction:column;gap:12px;">
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Seleccionar animal *</label>
            <select id="dc-ficha-animal" style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;background:#fff;"></select>
          </div>
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Veterinario responsable</label>
            <input id="dc-ficha-vet" type="text" placeholder="Nombre del veterinario" style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;">
          </div>
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Motivo de consulta</label>
            <input id="dc-ficha-motivo" type="text" placeholder="Ej: Control preventivo, Tratamiento..." style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;">
          </div>
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Observaciones</label>
            <textarea id="dc-ficha-obs" rows="3" style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;resize:none;"></textarea>
          </div>
        </div>
        <div style="display:flex;gap:10px;margin-top:20px;justify-content:flex-end;">
          <button onclick="document.getElementById('dc-m-ficha').style.display='none'" style="padding:10px 20px;border-radius:9px;border:1.5px solid #E0E8F4;background:#fff;color:#5A6A85;font-size:13px;font-weight:600;cursor:pointer;">Cancelar</button>
          <button onclick="dcGenerarFicha()" style="padding:10px 22px;border-radius:9px;border:none;background:#22C55E;color:#fff;font-size:13px;font-weight:600;cursor:pointer;">🖨️ Generar PDF</button>
        </div>
      </div>
    </div>`;
    document.body.appendChild(d.firstElementChild);
  }
  _dcSelectAnimales('dc-ficha-animal');
  document.getElementById('dc-m-ficha').style.display='flex';
}

async function dcGenerarFicha(){
  const arete=document.getElementById('dc-ficha-animal')?.value;
  if(!arete){toast('⚠️ Selecciona un animal.');return;}
  const animal=DB_ANIMALES.find(a=>a.arete===arete);
  if(!animal) return;
  const vet=document.getElementById('dc-ficha-vet')?.value||'—';
  const motivo=document.getElementById('dc-ficha-motivo')?.value||'Control general';
  const obs=document.getElementById('dc-ficha-obs')?.value||'';
  const rancho=SESSION?.rancho||'Mi Ganadería';
  const fecha=new Date().toLocaleDateString('es-PE',{day:'2-digit',month:'long',year:'numeric'});
  const vacunas=DB_SALUD.filter(s=>s.animal===arete);

  const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Ficha Veterinaria - ${arete}</title>
  <style>body{font-family:Arial,sans-serif;margin:0;padding:20px;color:#222;}
  .hdr{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #2E7DD6;padding-bottom:12px;margin-bottom:18px;}
  .titulo{font-size:20px;font-weight:800;color:#0D2B6B;}
  .rancho{font-size:14px;font-weight:700;color:#0D2B6B;}
  .sub{font-size:11px;color:#8FA3BF;}
  h2{font-size:15px;color:#0D2B6B;border-left:4px solid #2E7DD6;padding-left:10px;margin:16px 0 10px;}
  table{width:100%;border-collapse:collapse;margin-bottom:14px;}
  td{padding:7px 10px;font-size:12px;border:1px solid #E0E8F4;}
  td:first-child{background:#F8FAFF;font-weight:700;width:35%;}
  .vac-row{background:#F8FAFF;border-radius:6px;padding:8px 12px;margin-bottom:6px;font-size:12px;}
  .firma{display:flex;justify-content:space-between;margin-top:40px;}
  .firma-box{text-align:center;width:200px;}
  .firma-line{border-top:1px solid #0D2B6B;padding-top:6px;font-size:11px;color:#5A6A85;}
  .footer{text-align:center;margin-top:30px;font-size:10px;color:#8FA3BF;border-top:1px solid #E0E8F4;padding-top:10px;}
  @media print{body{padding:10px;}}</style></head>
  <body>
  <div class="hdr">
    <div><div class="titulo">FICHA VETERINARIA</div><div class="sub">Motivo: ${motivo}</div></div>
    <div style="text-align:right"><div class="rancho">${rancho}</div><div class="sub">Fecha: ${fecha}</div></div>
  </div>
  <h2>Datos del Animal</h2>
  <table>
    <tr><td>Arete</td><td>${animal.arete||'—'}</td></tr>
    <tr><td>Nombre</td><td>${animal.nombre||'—'}</td></tr>
    <tr><td>Raza</td><td>${animal.raza||'—'}</td></tr>
    <tr><td>Sexo</td><td>${animal.sexo||'—'}</td></tr>
    <tr><td>Fecha de nacimiento</td><td>${animal.nacimiento?fmtFecha(animal.nacimiento):'—'}</td></tr>
    <tr><td>Peso</td><td>${animal.peso?animal.peso+' kg':'—'}</td></tr>
    <tr><td>Estado</td><td>${animal.estado||'—'}</td></tr>
    <tr><td>Madre</td><td>${animal.madre||'—'}</td></tr>
    <tr><td>Padre</td><td>${animal.padre||'—'}</td></tr>
  </table>
  <h2>Veterinario Responsable</h2>
  <table><tr><td>Nombre</td><td>${vet}</td></tr></table>
  <h2>Historial de Salud</h2>
  ${vacunas.length?vacunas.map(v=>`<div class="vac-row"><strong>${v.tipo||'Vacuna'}</strong> — ${v.descripcion||''} · Fecha: ${v.fecha_aplicacion?fmtFecha(v.fecha_aplicacion):'—'} · Próxima: ${v.proxima_dosis?fmtFecha(v.proxima_dosis):'—'} ${v.veterinario?'· Vet: '+v.veterinario:''}</div>`).join('')
    :'<div style="color:#8FA3BF;font-size:12px;padding:8px;">Sin registros de salud.</div>'}
  ${obs?`<h2>Observaciones</h2><p style="font-size:12px;padding:8px;background:#F8FAFF;border-radius:6px;">${obs}</p>`:''}
  <div class="firma">
    <div class="firma-box"><div class="firma-line">Firma del Veterinario<br>${vet}</div></div>
    <div class="firma-box"><div class="firma-line">Propietario / Administrador<br>${SESSION?.nombre||''}</div></div>
  </div>
  <div class="footer">Generado por VaqueroApp · ${fecha} · ${rancho}</div>
  <script>window.onload=function(){window.print();};<\/script></body></html>`;

  const win=window.open('','_blank');
  if(win){win.document.write(html);win.document.close();}
  document.getElementById('dc-m-ficha').style.display='none';
  await _dcRegistrarHistorial('Ficha Veterinaria',`Ficha Veterinaria — ${arete}`,arete);
  toast('✅ Ficha Veterinaria generada');
}

/* ── Guía de Remisión ── */
function dcAbrirModalGuia(){
  if(!document.getElementById('dc-m-guia')){
    const d=document.createElement('div');
    d.innerHTML=`
    <div id="dc-m-guia" style="display:none;position:fixed;inset:0;background:rgba(13,43,107,.5);z-index:9999;align-items:center;justify-content:center;padding:16px;" onclick="if(event.target===this)this.style.display='none'">
      <div style="background:#fff;border-radius:18px;width:100%;max-width:500px;max-height:92vh;overflow-y:auto;padding:24px;box-shadow:0 20px 60px rgba(0,0,0,.2);">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;">
          <div style="font-family:'Montserrat',sans-serif;font-size:17px;font-weight:700;color:#0D2B6B;">🚚 Guía de Remisión</div>
          <button onclick="document.getElementById('dc-m-guia').style.display='none'" style="background:none;border:none;font-size:24px;cursor:pointer;color:#9eaaba;">×</button>
        </div>
        <div style="display:flex;flex-direction:column;gap:12px;">
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Destinatario *</label>
            <input id="dc-guia-dest" type="text" placeholder="Nombre del comprador/destinatario" style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;">
          </div>
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Dirección destino *</label>
            <input id="dc-guia-dir" type="text" placeholder="Dirección de destino" style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;">
          </div>
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Motivo de traslado</label>
            <input id="dc-guia-motivo" type="text" placeholder="Ej: Venta, traslado, feria..." style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;">
          </div>
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Placa del vehículo</label>
            <input id="dc-guia-placa" type="text" placeholder="Ej: ABC-123" style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;">
          </div>
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Conductor</label>
            <input id="dc-guia-conductor" type="text" placeholder="Nombre del conductor" style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;">
          </div>
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Seleccionar animales *</label>
            <div id="dc-guia-animales" style="border:1.5px solid #E0E8F4;border-radius:9px;max-height:180px;overflow-y:auto;margin-top:4px;"></div>
          </div>
        </div>
        <div style="display:flex;gap:10px;margin-top:20px;justify-content:flex-end;">
          <button onclick="document.getElementById('dc-m-guia').style.display='none'" style="padding:10px 20px;border-radius:9px;border:1.5px solid #E0E8F4;background:#fff;color:#5A6A85;font-size:13px;font-weight:600;cursor:pointer;">Cancelar</button>
          <button onclick="dcGenerarGuia()" style="padding:10px 22px;border-radius:9px;border:none;background:#2E7DD6;color:#fff;font-size:13px;font-weight:600;cursor:pointer;">🖨️ Generar PDF</button>
        </div>
      </div>
    </div>`;
    document.body.appendChild(d.firstElementChild);
  }
  // Poblar lista de animales con checkboxes
  const cont=document.getElementById('dc-guia-animales');
  if(cont){
    cont.innerHTML=DB_ANIMALES.filter(a=>a.estado!=='Muerto').map(a=>`
      <label style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:1px solid #F0F4FA;cursor:pointer;font-size:13px;">
        <input type="checkbox" value="${escH(a.arete)}" style="width:15px;height:15px;accent-color:#2E7DD6;flex-shrink:0;">
        <div>
          <strong>${escH(a.arete)}${a.nombre?' — '+escH(a.nombre):''}</strong>
          <span style="color:#8FA3BF;margin-left:6px;font-size:11px;">${escH(a.raza||'')} · ${a.sexo||''} · ${a.peso?a.peso+' kg':'—'}</span>
        </div>
      </label>`).join('');
  }
  document.getElementById('dc-m-guia').style.display='flex';
}

async function dcGenerarGuia(){
  const dest=document.getElementById('dc-guia-dest')?.value.trim();
  const dir=document.getElementById('dc-guia-dir')?.value.trim();
  if(!dest||!dir){toast('⚠️ Destinatario y dirección son obligatorios.');return;}

  const checks=[...document.querySelectorAll('#dc-guia-animales input[type="checkbox"]:checked')];
  if(!checks.length){toast('⚠️ Selecciona al menos un animal.');return;}
  const animalesSelec=checks.map(c=>DB_ANIMALES.find(a=>a.arete===c.value)).filter(Boolean);

  const motivo=document.getElementById('dc-guia-motivo')?.value||'—';
  const placa=document.getElementById('dc-guia-placa')?.value||'—';
  const conductor=document.getElementById('dc-guia-conductor')?.value||'—';
  const rancho=SESSION?.rancho||'Mi Ganadería';
  const fecha=new Date().toLocaleDateString('es-PE',{day:'2-digit',month:'long',year:'numeric'});
  const nro=String(Math.floor(Math.random()*9000)+1000);

  const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Guía de Remisión N° ${nro}</title>
  <style>body{font-family:Arial,sans-serif;margin:0;padding:20px;color:#222;}
  .hdr{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #2E7DD6;padding-bottom:12px;margin-bottom:18px;}
  h2{font-size:14px;color:#0D2B6B;border-left:4px solid #2E7DD6;padding-left:8px;margin:14px 0 8px;}
  table{width:100%;border-collapse:collapse;margin-bottom:12px;}
  td{padding:7px 10px;font-size:12px;border:1px solid #E0E8F4;}
  td:first-child{background:#F8FAFF;font-weight:700;width:35%;}
  th{background:#0D2B6B;color:#fff;padding:8px 10px;font-size:11px;text-align:left;}
  .firma{display:flex;justify-content:space-between;margin-top:40px;}
  .firma-box{text-align:center;width:200px;}
  .firma-line{border-top:1px solid #0D2B6B;padding-top:6px;font-size:11px;color:#5A6A85;}
  .footer{text-align:center;margin-top:20px;font-size:10px;color:#8FA3BF;border-top:1px solid #E0E8F4;padding-top:8px;}
  @media print{body{padding:10px;}}</style></head>
  <body>
  <div class="hdr">
    <div><div style="font-size:20px;font-weight:800;color:#0D2B6B;">GUÍA DE REMISIÓN</div><div style="font-size:12px;color:#8FA3BF;">N° ${nro}</div></div>
    <div style="text-align:right"><div style="font-size:14px;font-weight:700;color:#0D2B6B;">${rancho}</div><div style="font-size:11px;color:#8FA3BF;">Fecha: ${fecha}</div></div>
  </div>
  <h2>Datos del Traslado</h2>
  <table>
    <tr><td>Destinatario</td><td>${dest}</td></tr>
    <tr><td>Dirección destino</td><td>${dir}</td></tr>
    <tr><td>Motivo</td><td>${motivo}</td></tr>
    <tr><td>Placa del vehículo</td><td>${placa}</td></tr>
    <tr><td>Conductor</td><td>${conductor}</td></tr>
    <tr><td>Cantidad de animales</td><td>${animalesSelec.length}</td></tr>
  </table>
  <h2>Detalle de Animales</h2>
  <table>
    <thead><tr><th>Arete</th><th>Nombre</th><th>Raza</th><th>Sexo</th><th>Peso</th><th>Estado</th></tr></thead>
    <tbody>${animalesSelec.map(a=>`<tr>
      <td>${a.arete||'—'}</td><td>${a.nombre||'—'}</td><td>${a.raza||'—'}</td>
      <td>${a.sexo||'—'}</td><td>${a.peso?a.peso+' kg':'—'}</td><td>${a.estado||'—'}</td>
    </tr>`).join('')}</tbody>
  </table>
  <div class="firma">
    <div class="firma-box"><div class="firma-line">Firma del Remitente<br>${SESSION?.nombre||''}</div></div>
    <div class="firma-box"><div class="firma-line">Firma del Receptor<br>${dest}</div></div>
  </div>
  <div class="footer">Generado por VaqueroApp · ${fecha} · ${rancho}</div>
  <script>window.onload=function(){window.print();};<\/script></body></html>`;
  ...
  const win=window.open('','_blank');
  if(win){win.document.write(html);win.document.close();}
  document.getElementById('dc-m-guia').style.display='none';
  await _dcRegistrarHistorial('Guía de Remisión',`Guía de Remisión N°${nro} — ${animalesSelec.length} animales`,'');
  toast(`✅ Guía de Remisión generada — ${animalesSelec.length} animal(es)`);
}

/* ── Certificado Sanitario ── */
function dcAbrirModalCertificado(){
  if(!document.getElementById('dc-m-cert')){
    const d=document.createElement('div');
    d.innerHTML=`
    <div id="dc-m-cert" style="display:none;position:fixed;inset:0;background:rgba(13,43,107,.5);z-index:9999;align-items:center;justify-content:center;padding:16px;" onclick="if(event.target===this)this.style.display='none'">
      <div style="background:#fff;border-radius:18px;width:100%;max-width:480px;max-height:90vh;overflow-y:auto;padding:24px;box-shadow:0 20px 60px rgba(0,0,0,.2);">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;">
          <div style="font-family:'Montserrat',sans-serif;font-size:17px;font-weight:700;color:#0D2B6B;">📋 Certificado Sanitario</div>
          <button onclick="document.getElementById('dc-m-cert').style.display='none'" style="background:none;border:none;font-size:24px;cursor:pointer;color:#9eaaba;">×</button>
        </div>
        <div style="display:flex;flex-direction:column;gap:12px;">
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Seleccionar animal *</label>
            <select id="dc-cert-animal" style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;background:#fff;"></select>
          </div>
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Veterinario responsable</label>
            <input id="dc-cert-vet" type="text" placeholder="Nombre del veterinario" style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;">
          </div>
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Colegiatura / Reg. Profesional</label>
            <input id="dc-cert-coleg" type="text" placeholder="N° de colegiatura" style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;">
          </div>
          <div>
            <label style="font-size:11px;font-weight:700;color:#5A6A85;text-transform:uppercase;">Observaciones adicionales</label>
            <textarea id="dc-cert-obs" rows="3" placeholder="Observaciones del veterinario..." style="width:100%;margin-top:4px;border:1.5px solid #E0E8F4;border-radius:9px;padding:9px 12px;font-size:13px;outline:none;box-sizing:border-box;resize:none;"></textarea>
          </div>
        </div>
        <div style="display:flex;gap:10px;margin-top:20px;justify-content:flex-end;">
          <button onclick="document.getElementById('dc-m-cert').style.display='none'" style="padding:10px 20px;border-radius:9px;border:1.5px solid #E0E8F4;background:#fff;color:#5A6A85;font-size:13px;font-weight:600;cursor:pointer;">Cancelar</button>
          <button onclick="dcGenerarCertificado()" style="padding:10px 22px;border-radius:9px;border:none;background:#F0A500;color:#fff;font-size:13px;font-weight:600;cursor:pointer;">🖨️ Generar PDF</button>
        </div>
      </div>
    </div>`;
    document.body.appendChild(d.firstElementChild);
  }
  _dcSelectAnimales('dc-cert-animal');
  document.getElementById('dc-m-cert').style.display='flex';
}

async function dcGenerarCertificado(){
  const arete=document.getElementById('dc-cert-animal')?.value;
  if(!arete){toast('⚠️ Selecciona un animal.');return;}
  const animal=DB_ANIMALES.find(a=>a.arete===arete);
  if(!animal) return;
  const vet=document.getElementById('dc-cert-vet')?.value||'—';
  const coleg=document.getElementById('dc-cert-coleg')?.value||'—';
  const obs=document.getElementById('dc-cert-obs')?.value||'Animal aparentemente sano al momento de la inspección.';
  const rancho=SESSION?.rancho||'Mi Ganadería';
  const fecha=new Date().toLocaleDateString('es-PE',{day:'2-digit',month:'long',year:'numeric'});
  const nro=String(Math.floor(Math.random()*9000)+1000);
  const vacunas=DB_SALUD.filter(s=>s.animal===arete);

  const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Certificado Sanitario - ${arete}</title>
  <style>body{font-family:Arial,sans-serif;margin:0;padding:20px;color:#222;}
  .hdr{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #F0A500;padding-bottom:12px;margin-bottom:18px;}
  .cert-badge{background:#FFF3E0;border:2px solid #F0A500;border-radius:12px;padding:10px 18px;text-align:center;margin-bottom:16px;}
  h2{font-size:14px;color:#0D2B6B;border-left:4px solid #F0A500;padding-left:8px;margin:14px 0 8px;}
  table{width:100%;border-collapse:collapse;margin-bottom:12px;}
  td{padding:7px 10px;font-size:12px;border:1px solid #E0E8F4;}
  td:first-child{background:#FFF8EE;font-weight:700;width:40%;}
  .obs-box{background:#FFF8EE;border:1px solid #F0A500;border-radius:8px;padding:12px;font-size:12px;margin:10px 0;}
  .firma{display:flex;justify-content:center;margin-top:40px;}
  .firma-box{text-align:center;width:240px;}
  .firma-line{border-top:2px solid #0D2B6B;padding-top:6px;font-size:11px;color:#5A6A85;}
  .footer{text-align:center;margin-top:20px;font-size:10px;color:#8FA3BF;border-top:1px solid #E0E8F4;padding-top:8px;}
  @media print{body{padding:10px;}}</style></head>
  <body>
  <div class="hdr">
    <div><div style="font-size:20px;font-weight:800;color:#0D2B6B;">CERTIFICADO SANITARIO</div><div style="font-size:12px;color:#8FA3BF;">N° CS-${nro}</div></div>
    <div style="text-align:right"><div style="font-size:14px;font-weight:700;color:#0D2B6B;">${rancho}</div><div style="font-size:11px;color:#8FA3BF;">Fecha: ${fecha}</div></div>
  </div>
  <div class="cert-badge">
    <div style="font-size:13px;font-weight:700;color:#e07b00;">🏅 CERTIFICADO OFICIAL DE ESTADO SANITARIO</div>
    <div style="font-size:11px;color:#8FA3BF;margin-top:4px;">El animal descrito a continuación ha sido inspeccionado por el veterinario responsable.</div>
  </div>
  <h2>Datos del Animal</h2>
  <table>
    <tr><td>Arete</td><td>${animal.arete||'—'}</td></tr>
    <tr><td>Nombre</td><td>${animal.nombre||'—'}</td></tr>
    <tr><td>Especie / Raza</td><td>Bovino / ${animal.raza||'—'}</td></tr>
    <tr><td>Sexo</td><td>${animal.sexo||'—'}</td></tr>
    <tr><td>Fecha de nacimiento</td><td>${animal.nacimiento?fmtFecha(animal.nacimiento):'—'}</td></tr>
    <tr><td>Peso aproximado</td><td>${animal.peso?animal.peso+' kg':'—'}</td></tr>
    <tr><td>Estado actual</td><td>${animal.estado||'—'}</td></tr>
  </table>
  <h2>Vacunas Aplicadas</h2>
  ${vacunas.length?`<table><thead><tr style="background:#0D2B6B;color:#fff;"><th style="padding:6px 8px;font-size:11px;">Tipo</th><th style="padding:6px 8px;font-size:11px;">Descripción</th><th style="padding:6px 8px;font-size:11px;">Fecha</th><th style="padding:6px 8px;font-size:11px;">Próxima</th></tr></thead><tbody>
    ${vacunas.map(v=>`<tr><td>${v.tipo||'—'}</td><td>${v.descripcion||'—'}</td><td>${v.fecha_aplicacion?fmtFecha(v.fecha_aplicacion):'—'}</td><td>${v.proxima_dosis?fmtFecha(v.proxima_dosis):'—'}</td></tr>`).join('')}
    </tbody></table>`:'<div style="color:#8FA3BF;font-size:12px;padding:8px;">Sin vacunas registradas.</div>'}
  <h2>Dictamen Sanitario</h2>
  <div class="obs-box">${obs}</div>
  <h2>Responsable de la Inspección</h2>
  <table>
    <tr><td>Veterinario</td><td>${vet}</td></tr>
    <tr><td>Colegiatura / Reg.</td><td>${coleg}</td></tr>
    <tr><td>Fecha de emisión</td><td>${fecha}</td></tr>
  </table>
  <div class="firma">
    <div class="firma-box">
      <div style="margin-bottom:40px;"></div>
      <div class="firma-line">Firma y Sello del Veterinario<br><strong>${vet}</strong><br>Reg. ${coleg}</div>
    </div>
  </div>
  <div class="footer">Generado por VaqueroApp · ${fecha} · ${rancho} · Este documento tiene validez como certificado sanitario.</div>
  <script>window.onload=function(){window.print();};<\/script></body></html>`;

  const win=window.open('','_blank');
  if(win){win.document.write(html);win.document.close();}
  document.getElementById('dc-m-cert').style.display='none';
  await _dcRegistrarHistorial('Certificado Sanitario',`Certificado Sanitario N°CS-${nro} — ${arete}`,arete);
  toast('✅ Certificado Sanitario generado');
}

/* ── Registrar en historial ── */
async function _dcRegistrarHistorial(tipo, nombre, animal){
  if(!SESSION?.rancho_id) return;
  ...
  try{
    const res=await fetch(`${SB_URL}/rest/v1/historial_docs`,{
      method:'POST', headers:SB_HEADERS,
      body:JSON.stringify({
        rancho_id:   String(SESSION.rancho_id),
        user_id:     SESSION.user_id,
    ...
    const data=await res.json();
    console.log('[historial_docs]', res.status, data);
    if(!res.ok) console.error('[historial_docs error]', data.message||data.hint||data.details);
    await cargarDocumentos();
  }catch(e){ console.warn('[Docs]',e); }
}
...
let _imArchivo    = null;
let _imDatosPreview = [];
let _imTipoActual = 'animales';

/* ── Actualizar pasos visuales ── */
function imActualizarPasos(pasoActual){
  const nums = document.querySelectorAll('#sec-importar .im-paso-num');
  const icos = document.querySelectorAll('#sec-importar .im-paso-ico');
  nums.forEach((el,i)=>{
    const paso = i+1;
    el.className = 'im-paso-num ' + (paso < pasoActual ? 'done' : paso === pasoActual ? 'active' : 'pending');
    if(paso < pasoActual){
      el.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" width="18" height="18"><path d="M5 13l4 4L19 7"/></svg>';
    } else {
      el.textContent = paso;
    }
    if(icos[i]){
      icos[i].style.background = paso < pasoActual ? '#d1fae5' : paso === pasoActual ? '#dbeafe' : '#f0f4fb';
      const svg = icos[i].querySelector('svg');
      if(svg) svg.style.stroke = paso < pasoActual ? '#059669' : paso === pasoActual ? '#2E7DD6' : '#9eaaba';
    }
  });
}

function imInicializar(){
  imActualizarPasos(1);

  // Sobreescribir las funciones del script inline del HTML inyectado
  setTimeout(()=>{

    window.imDescargarPlantilla = function(){
      if(!window.XLSX){ toast('❌ Librería Excel no disponible.'); return; }
      const cols=['arete','nombre','raza','sexo','nacimiento','peso','estado','madre','padre','observaciones'];
      const ejemplo=[
        ['A-001','Lolita','Brahman','Hembra','2023-01-15','250','Activo','A-000','—',''],
        ['A-002','Toro1','Angus','Macho','2022-06-20','400','Activo','—','—','']
      ];
      ...
      imActualizarPasos(2);
      toast('📥 Plantilla descargada — llénala y súbela');
    };

    window.imSeleccionarArchivo = function(input){
      if(!input.files||!input.files[0]) return;
      const file=input.files[0];
      const nameEl=document.getElementById('im-file-name');
      const sizeEl=document.getElementById('im-file-size');
      const prevEl=document.getElementById('im-file-preview');
      const dropEl=document.getElementById('im-drop-zone');
      const btnEl =document.getElementById('im-btn-validar');
      if(nameEl) nameEl.textContent=file.name;
      if(sizeEl) sizeEl.textContent=(file.size/1024).toFixed(1)+' KB';
      if(prevEl) prevEl.classList.add('show');
      if(dropEl) dropEl.style.display='none';
      if(btnEl){ btnEl.disabled=false; btnEl.classList.add('ready'); }
      imActualizarPasos(3);
      imCargarArchivo(file);
    };

    window.imDrop = function(e){
      e.preventDefault();
      document.getElementById('im-drop-zone')?.classList.remove('dragover');
      const files=e.dataTransfer.files;
      if(files&&files[0]){
        const input=document.getElementById('im-file-input');
        try{
          const dt=new DataTransfer();
          dt.items.add(files[0]);
          input.files=dt.files;
        }catch(err){}
        window.imSeleccionarArchivo({files:[files[0]]});
      }
    };

    window.imQuitarArchivo = function(){
      const input=document.getElementById('im-file-input');
      const prevEl=document.getElementById('im-file-preview');
      const dropEl=document.getElementById('im-drop-zone');
      const btnEl =document.getElementById('im-btn-validar');
      ...
      if(btnEl){ btnEl.disabled=true; btnEl.classList.remove('ready'); }
      _imArchivo=null; _imDatosPreview=[];
      const prev=document.getElementById('im-preview');
      if(prev){prev.innerHTML='';prev.style.display='none';}
      imActualizarPasos(1);
      toast('🗑 Archivo eliminado');
    };

    window.imValidar = function(){
      imActualizarPasos(4);
      imValidarReal();
    };
...
/* ── Descargar plantilla Excel (versión global) ── */
function imDescargarPlantilla(){
  if(!window.XLSX){ toast('❌ Librería Excel no disponible.'); return; }

  const tipo = document.querySelector('#sec-importar .im-tipo-sel.on, #sec-importar [data-tipo].on')?.dataset?.tipo || 'animales';
  const plantillas = {
    animales: {
      nombre: 'plantilla_animales.xlsx',
      cols: ['arete','nombre','raza','sexo','nacimiento','peso','estado','madre','padre','observaciones'],
      ejemplo: [['A-001','Lolita','Brahman','Hembra','2023-01-15','250','Activo','A-000','—',''],
                ['A-002','Toro1','Angus','Macho','2022-06-20','400','Activo','—','—','']]
    },
    salud: {
      nombre: 'plantilla_salud.xlsx',
      cols: ['animal','tipo','descripcion','dosis','fecha_aplicacion','proxima_dosis','veterinario','costo'],
      ejemplo: [['A-001','Vacuna','Aftosa','5ml','2024-01-10','2024-07-10','Dr. García','25'],
                ['A-001','Desparasitación','Ivermectina','3ml','2024-02-01','2024-05-01','','15']]
    },
    gastos: {
      nombre: 'plantilla_gastos.xlsx',
      cols: ['fecha','descripcion','tipo','monto','es_ingreso','animal','observaciones'],
      ejemplo: [['2024-01-15','Compra alimento','Alimentación','500','false','',''],
                ['2024-01-20','Venta novillo','Venta de animales','2000','true','A-002','']]
    },
  };

  const pl = plantillas[tipo] || plantillas.animales;
  const ws = XLSX.utils.aoa_to_sheet([pl.cols, ...pl.ejemplo]);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Datos');
  XLSX.writeFile(wb, pl.nombre);
  toast(`📥 Plantilla "${pl.nombre}" descargada`);
}

/* ── Cargar y parsear archivo ── */
async function imCargarArchivo(file){
  _imArchivo = file;
  const ext = file.name.split('.').pop().toLowerCase();

  // Mostrar info del archivo con IDs reales
  const filePreview = document.getElementById('im-file-preview');
  const fileName    = document.getElementById('im-file-name');
  const fileSize    = document.getElementById('im-file-size');
  ...
  try{
    ...
    if(ext === 'csv'){
      ...
      toast('⚠️ Solo se aceptan archivos .xlsx, .xls o .csv'); return;
    }

    rows = rows.filter(r => r.some(c => String(c||'').trim()));
    if(rows.length < 2){ toast('⚠️ El archivo está vacío o solo tiene encabezados.'); return; }

    _imDatosPreview = rows;
    imMostrarPreview(rows);

    // Remover atributo disabled y agregar clase ready
    const btnVal = document.getElementById('im-btn-validar');
    if(btnVal){
      btnVal.removeAttribute('disabled');
      btnVal.classList.add('ready');
      btnVal.style.cursor = 'pointer';
      btnVal.style.background = '#2E7DD6';
      btnVal.style.color = '#fff';
    }

    toast(`✅ Archivo cargado: ${rows.length-1} registros detectados`);
  }catch(e){
...
/* ── Mostrar preview ── */
function imMostrarPreview(rows){
  // Usar el contenedor correcto del HTML original
  let prev = document.getElementById('im-preview');
  if(!prev){
    // Crear si no existe — insertar después de im-file-preview
    prev = document.createElement('div');
    prev.id = 'im-preview';
    prev.style.cssText = 'margin-top:14px;';
    const fp = document.getElementById('im-file-preview');
    if(fp) fp.parentNode.insertBefore(prev, fp.nextSibling);
    else {
      const zona = document.getElementById('im-drop-zone');
      if(zona) zona.parentNode.insertBefore(prev, zona.nextSibling);
    }
  }

  const headers = rows[0] || [];
  const datos   = rows.slice(1, 6);
  const total   = rows.length - 1;

  // Detectar tipo
  const hLower = headers.map(h=>String(h).toLowerCase().trim());
  let tipo = 'animales';
  if(hLower.includes('animal') && hLower.includes('tipo')) tipo = 'salud';
  else if(hLower.includes('monto') || hLower.includes('es_ingreso')) tipo = 'gastos';

  const tipoLabel = {animales:'🐄 Animales', salud:'💉 Salud/Vacunas', gastos:'💰 Gastos'}[tipo];

  prev.innerHTML = `
    <div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:10px;padding:12px 16px;margin-bottom:10px;display:flex;align-items:center;gap:12px;">
      <span style="font-size:24px;">📊</span>
      <div>
        <div style="font-family:'Montserrat',sans-serif;font-size:14px;font-weight:700;color:#1D4ED8;">
          ${total} registros detectados ${tipoLabel}
        </div>
        <div style="font-size:12px;color:#3B82F6;">${headers.length} columnas · Vista previa de los primeros 5</div>
      </div>
    </div>
    <div style="overflow-x:auto;border-radius:8px;border:1px solid #E0E8F4;">
      <table style="width:100%;border-collapse:collapse;font-size:12px;">
        <thead>
          <tr style="background:#F8FAFF;">
            ${headers.map(h=>`<th style="padding:8px 12px;text-align:left;font-weight:700;color:#0D2B6B;white-space:nowrap;border-bottom:1px solid #E0E8F4;border-right:1px solid #F0F4FA;">${escH(String(h))}</th>`).join('')}
          </tr>
        </thead>
        <tbody>
          ${datos.map((row,ri)=>`<tr style="background:${ri%2?'#FAFAFA':'#fff'};border-bottom:1px solid #F0F4FA;">
            ${headers.map((_,ci)=>`<td style="padding:7px 12px;color:#5A6A85;white-space:nowrap;border-right:1px solid #F0F4FA;">${escH(String(row[ci]||''))}</td>`).join('')}
          </tr>`).join('')}
        </tbody>
      </table>
    </div>
    ${total > 5 ? `<div style="font-size:11px;color:#8FA3BF;margin-top:6px;text-align:right;">... y ${total-5} registros más</div>` : ''}
  `;
...
function imQuitarArchivo(){
  ...
  if(btnVal){
    ...
    btnVal.classList.remove('ready');
    btnVal.style.cursor='not-allowed';
    btnVal.style.background='';
  }
  toast('🗑 Archivo eliminado');
}

/* ── Validar (solo muestra confirmación, NO importa aún) ── */
async function imValidarReal(){
  if(!_imDatosPreview.length){ toast('⚠️ Primero carga un archivo.'); return; }

  const headers = _imDatosPreview[0].map(h=>String(h).toLowerCase().trim());
  const filas   = _imDatosPreview.slice(1).filter(r=>r.some(c=>c));

  // Detectar tipo
  let tipo = 'animales';
  if(headers.includes('animal') && headers.includes('tipo')) tipo='salud';
  else if(headers.includes('monto') || headers.includes('es_ingreso')) tipo='gastos';

  const tipoLabel = {animales:'🐄 Animales', salud:'💉 Salud/Vacunas', gastos:'💰 Gastos'}[tipo];

  // Mostrar resumen de validación — NO importar aún
  const prev = document.getElementById('im-preview');
  if(prev){
    prev.innerHTML += `
      <div style="margin-top:14px;padding:14px 16px;border-radius:10px;background:#F0FFF4;border:1px solid #86EFAC;">
        <div style="font-family:'Montserrat',sans-serif;font-size:14px;font-weight:700;color:#166534;margin-bottom:6px;">
          ✅ Validación correcta — ${filas.length} registros de ${tipoLabel} listos para importar
        </div>
        <div style="font-size:12px;color:#15803D;">Haz clic en "Importar datos" para guardar en Supabase.</div>
      </div>`;
  }

  // Cambiar botón validar a "Importar datos" ahora sí
  const btnVal = document.getElementById('im-btn-validar');
  if(btnVal){
    btnVal.textContent = '⬆️ Importar datos';
    btnVal.onclick = () => imEjecutarImportacion(tipo, filas, headers);
    btnVal.style.background = '#22C55E';
  }

  toast(`✅ Validados ${filas.length} registros. Ahora haz clic en "Importar datos".`);
}

/* ── Importar (se ejecuta al 2do clic) ── */
async function imEjecutarImportacion(tipo, filas, headers){
  if(!SESSION?.rancho_id){ toast('⚠️ Sin rancho asignado.'); return; }

  const btnVal = document.getElementById('im-btn-validar');
  if(btnVal){ btnVal.textContent='⏳ Importando...'; btnVal.disabled=true; }

  let ok=0, err=0;

  for(const fila of filas){
    const obj={};
    headers.forEach((h,i)=>{ if(h) obj[h]=fila[i]||null; });
    let payload={rancho_id:SESSION.rancho_id};

    if(tipo==='animales'){
      if(!obj.arete){ err++; continue; }
      payload={...payload, arete:obj.arete, nombre:obj.nombre||null, raza:obj.raza||null,
        sexo:obj.sexo||null, nacimiento:obj.nacimiento||null,
        peso:obj.peso?parseFloat(obj.peso):null,
        estado:obj.estado||'Activo', madre:obj.madre||null, padre:obj.padre||null,
        observaciones:obj.observaciones||null};
    } else if(tipo==='salud'){
      if(!obj.animal){ err++; continue; }
      payload={...payload, animal:obj.animal, tipo:obj.tipo||'Vacuna',
        descripcion:obj.descripcion||null, dosis:obj.dosis||null,
        fecha_aplicacion:obj.fecha_aplicacion||null,
        proxima_dosis:obj.proxima_dosis||null,
        veterinario:obj.veterinario||null,
        costo:obj.costo?parseFloat(obj.costo):null};
    } else if(tipo==='gastos'){
      if(!obj.monto){ err++; continue; }
      payload={...payload, descripcion:obj.descripcion||'Importado',
        tipo:obj.tipo||null, monto:parseFloat(obj.monto)||0,
        es_ingreso:obj.es_ingreso==='true'||obj.es_ingreso==='1'||obj.es_ingreso==='Sí',
        fecha:obj.fecha||new Date().toISOString().split('T')[0],
        animal:obj.animal||null};
    }

    try{
      const tabla=tipo==='salud'?'salud':tipo;
  ...
  // Recargar datos
  if(tipo==='animales') await cargarAnimales().catch(()=>{});
  else if(tipo==='salud') await cargarSalud().catch(()=>{});
  else if(tipo==='gastos') await cargarGastos().catch(()=>{});

  const msg=`✅ Importación completada: ${ok} registros importados${err?`, ${err} con error`:''}`;

  const prev=document.getElementById('im-preview');
  if(prev) prev.innerHTML+=`<div style="margin-top:10px;padding:12px 16px;border-radius:10px;background:${err?'#FFF3CD':'#D1FAE5'};color:${err?'#92400E':'#065F46'};font-size:13px;font-weight:600;">${msg}</div>`;

  if(btnVal){
    btnVal.textContent='✅ Importación completada';
    btnVal.disabled=true;
    btnVal.style.background='#6B7280';
  }
...
async function exInicializar(){
  ...
  const [cA,cS,cG,cI] = await Promise.all([
    DB_ANIMALES.length   ? Promise.resolve(DB_ANIMALES.length)   : sg('animales'),
    DB_SALUD.length      ? Promise.resolve(DB_SALUD.length)      : sg('salud'),
    DB_GASTOS.length     ? Promise.resolve(DB_GASTOS.length)     : sg('gastos'),
    DB_INVENTARIO.length ? Promise.resolve(DB_INVENTARIO.length) : sg('inventario'),
  ]);

  // inyectarExportar() recrea el HTML — actualizar TODOS los ex-stat-num por posición
  const actualizar = () => {
    // Sobreescribir exDescargar
    window.exDescargar = exDescargar;

    // Los 4 contadores por clase (el HTML inyectado no tiene IDs)
    const nums = document.querySelectorAll('#sec-exportar .ex-stat-num');
    const vals = [cA, cS, cG, cI];
    nums.forEach((el,i) => { if(vals[i]!==undefined) el.textContent = vals[i]; });

    // También intentar por ID si los tiene
    const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v;};
  ...
  // Intentar varias veces por si el HTML todavía no se inyectó
  actualizar();
...
async function exDescargar(tipo){
  ...
  if(!DB_SALUD.length)      DB_SALUD      = await sg('salud');
  if(!DB_GASTOS.length)     DB_GASTOS     = await sg('gastos');
  if(!DB_INVENTARIO.length) DB_INVENTARIO = await sg('inventario');
  if(!DB_INSEM.length)      DB_INSEM      = await sg('insem');
  if(!DB_PARTOS.length)     DB_PARTOS     = await sg('partos');
  ...
  const addHoja = (nombre, datos, cols) => {
    if(!datos.length){
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([cols,['Sin datos']]), nombre);
      return;
    }
  ...
  switch(tipo){
    case 'animales':
      addHoja('Animales', DB_ANIMALES,
        ['arete','nombre','raza','sexo','nacimiento','peso','estado','madre','padre','observaciones']);
      XLSX.writeFile(wb, `${rancho}_animales_${fecha}.xlsx`);
      toast(`✅ ${DB_ANIMALES.length} animales exportados`);
      break;

    case 'salud':
      addHoja('Salud & Vacunas', DB_SALUD,
        ['animal','tipo','descripcion','dosis','fecha_aplicacion','proxima_dosis','veterinario','costo','observaciones']);
      XLSX.writeFile(wb, `${rancho}_salud_${fecha}.xlsx`);
      toast(`✅ ${DB_SALUD.length} registros de salud exportados`);
      break;

    case 'gastos':
      addHoja('Gastos & Ingresos', DB_GASTOS,
        ['fecha','descripcion','tipo','monto','es_ingreso','animal','observaciones']);
      XLSX.writeFile(wb, `${rancho}_gastos_${fecha}.xlsx`);
      toast(`✅ ${DB_GASTOS.length} movimientos exportados`);
      break;

    case 'inventario':
      addHoja('Inventario', DB_INVENTARIO,
        ['nombre','categoria','unidad','stock','minimo','precio','proveedor','vencimiento','observaciones']);
      XLSX.writeFile(wb, `${rancho}_inventario_${fecha}.xlsx`);
      toast(`✅ ${DB_INVENTARIO.length} productos exportados`);
      break;
    ...
    case 'completo':
      addHoja('Animales', DB_ANIMALES,
        ['arete','nombre','raza','sexo','nacimiento','peso','estado','madre','padre','observaciones']);
      addHoja('Salud & Vacunas', DB_SALUD,
        ['animal','tipo','descripcion','dosis','fecha_aplicacion','proxima_dosis','veterinario','costo']);
      addHoja('Inseminaciones', DB_INSEM,
        ['hembra','fecha','toro_semen','tecnica','tecnico','parto_estimado','resultado','costo']);
      addHoja('Partos', DB_PARTOS,
        ['fecha','madre','arete_cria','sexo_cria','peso_nacimiento','tipo_parto','estado_cria','padre']);
      addHoja('Gastos & Ingresos', DB_GASTOS,
        ['fecha','descripcion','tipo','monto','es_ingreso','animal','observaciones']);
      addHoja('Inventario', DB_INVENTARIO,
        ['nombre','categoria','unidad','stock','minimo','precio','proveedor','vencimiento']);
      /* Hoja resumen */
      const anyo=new Date().getFullYear();
      const wsRes=XLSX.utils.json_to_sheet([
        {'Indicador':'Rancho',             'Valor':SESSION?.rancho||''},
        {'Indicador':'Fecha exportación',  'Valor':fecha},
        {'Indicador':'Total animales',     'Valor':DB_ANIMALES.length},
        {'Indicador':'Activos',            'Valor':DB_ANIMALES.filter(a=>a.estado==='Activo').length},
        {'Indicador':'Gestantes',          'Valor':DB_ANIMALES.filter(a=>a.estado==='Gestante').length},
        {'Indicador':'Partos este año',    'Valor':DB_PARTOS.filter(p=>(p.fecha||'').startsWith(anyo+'')).length},
        {'Indicador':'Inseminaciones',     'Valor':DB_INSEM.length},
        {'Indicador':'Tasa de preñez',     'Valor':DB_INSEM.length?Math.round(DB_INSEM.filter(i=>i.resultado==='Preñada').length/DB_INSEM.length*100)+'%':'0%'},
        {'Indicador':'Registros de salud', 'Valor':DB_SALUD.length},
        {'Indicador':'Productos inventario','Valor':DB_INVENTARIO.length},
        {'Indicador':'Total ingresos',     'Valor':'S/ '+DB_GASTOS.filter(g=>g.es_ingreso).reduce((s,g)=>s+parseFloat(g.monto||0),0).toFixed(2)},
        {'Indicador':'Total gastos',       'Valor':'S/ '+DB_GASTOS.filter(g=>!g.es_ingreso).reduce((s,g)=>s+parseFloat(g.monto||0),0).toFixed(2)},
      ]);
      XLSX.utils.book_append_sheet(wb, wsRes, 'Resumen');
      XLSX.writeFile(wb, `${rancho}_backup_completo_${fecha}.xlsx`);
      const total=DB_ANIMALES.length+DB_SALUD.length+DB_INSEM.length+DB_PARTOS.length+DB_GASTOS.length+DB_INVENTARIO.length;
...
/* Helper local por si escH no está disponible */
const _escH = s => typeof escH==='function' ? _escH(s) : String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
...
function _busRenderGrid(lista){
  ...
  console.log('[BuscarGrid] lista:', lista.length, 'grid:', grid?.id, 'parent:', grid?.parentElement?.id||grid?.parentElement?.className);
  if(!grid) return;

  if(!lista.length){
    const totalAnim=(typeof DB_ANIMALES!=='undefined'&&DB_ANIMALES)?DB_ANIMALES.length:0;
    grid.innerHTML=`<div style="text-align:center;padding:50px;color:#8FA3BF;grid-column:1/-1;">
      <div style="font-size:48px;margin-bottom:10px;">${_busQuery?'🔍':'🐄'}</div>
      <div style="font-size:14px;font-weight:600;color:#5A6A85;margin-bottom:6px;">
        ${_busQuery?`Sin resultados para "${_escH(_busQuery)}"`:totalAnim?'Selecciona un filtro o escribe para buscar':'Sin animales registrados'}
      </div>
  ...
  const hl=txt=>{
    if(!_busQuery||!txt) return _escH(txt||'');
    const re=new RegExp(`(${_busQuery.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')})`, 'gi');
    return _escH(txt).replace(re,'<mark class="bus-highlight">$1</mark>');
  };

  grid.innerHTML=lista.map(a=>{
    const b=BADGE[a.estado]||{bg:'#F3F4F6',color:'#374151'};
    const inicial=(a.raza||a.nombre||'?').charAt(0).toUpperCase();
    return `<div onclick="busVerAnimal('${a.id}')" style="background:#fff;border:1px solid #E8EEF8;border-radius:14px;padding:14px;cursor:pointer;display:flex;gap:12px;align-items:flex-start;transition:all .2s;" onmouseover="this.style.borderColor='#2E7DD6';this.style.boxShadow='0 4px 16px rgba(46,125,214,.12)'" onmouseout="this.style.borderColor='#E8EEF8';this.style.boxShadow='none'">
      <div style="width:50px;height:50px;border-radius:12px;background:#EFF6FF;display:flex;align-items:center;justify-content:center;font-family:'Montserrat',sans-serif;font-size:20px;font-weight:800;color:#2E7DD6;flex-shrink:0;overflow:hidden;">
        ${a.foto?`<img src="${_escH(a.foto)}" style="width:100%;height:100%;object-fit:cover;" onerror="this.parentNode.textContent='${inicial}'">`:`${inicial}`}
      </div>
      <div style="flex:1;min-width:0;">
        <div style="font-size:11px;color:#8FA3BF;font-weight:600;">Arete: ${hl(a.arete)}</div>
        <div style="font-family:'Montserrat',sans-serif;font-size:14px;font-weight:700;color:#0D2B6B;margin:2px 0;">${hl(a.nombre||'Sin nombre')}</div>
      ...
      <span style="background:${b.bg};color:${b.color};font-size:10px;font-weight:700;padding:3px 9px;border-radius:20px;white-space:nowrap;flex-shrink:0;">${a.estado||'—'}</span>
    </div>`;
  }).join('');
}
...
/* ══════════════════════════════════════════
   MÓDULO ÁRBOL GENEALÓGICO
   ══════════════════════════════════════════ */

let _arbolAnimalSel = null;

function _crearSecArbol(){
  if(document.getElementById('sec-arbol')) return;
  const main=document.querySelector('.d-main')||document.querySelector('main')||document.body;
  const div=document.createElement('div');
  div.id='sec-arbol';
  div.style.cssText='display:none;flex-direction:column;overflow-y:auto;overflow-x:hidden;flex:1;padding:0;background:#F8FAFF;';
  div.innerHTML=`
    <!-- Header -->
    <div style="background:linear-gradient(135deg,#0D2B6B,#1a4a9e);padding:24px 28px;color:#fff;">
      <div style="font-family:'Montserrat',sans-serif;font-size:22px;font-weight:800;margin-bottom:4px;">🌳 Árbol Genealógico</div>
      <div style="font-size:13px;opacity:.75;margin-bottom:16px;">Visualiza el linaje y parentesco de tus animales</div>
      <div style="position:relative;max-width:500px;">
        <input id="arbol-buscar" type="text" placeholder="Buscar animal por arete o nombre..."
          oninput="arbolBuscar(this.value)"
          style="width:100%;padding:11px 44px 11px 16px;border-radius:10px;border:none;font-size:14px;outline:none;box-sizing:border-box;">
        <span style="position:absolute;right:14px;top:50%;transform:translateY(-50%);font-size:16px;">🔍</span>
      </div>
      <!-- Sugerencias -->
      <div id="arbol-sugerencias" style="display:none;position:absolute;z-index:99;background:#fff;border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,.15);max-width:500px;max-height:200px;overflow-y:auto;margin-top:4px;"></div>
    </div>

    <!-- Contenido -->
    <div style="padding:20px 28px;flex:1;">
      <div id="arbol-cont" style="min-height:400px;">
        <div style="text-align:center;padding:60px 20px;color:#8FA3BF;">
          <div style="font-size:56px;margin-bottom:12px;">🌳</div>
          <div style="font-size:15px;font-weight:600;color:#5A6A85;">Busca un animal para ver su árbol genealógico</div>
          <div style="font-size:12px;margin-top:6px;">Se mostrarán sus padres, abuelos y descendencia</div>
        </div>
      </div>
    </div>`;
  main.appendChild(div);
...
async function cargarArbol(){
  ...
  if(!DB_ANIMALES.length){
    const rid=await _asegurarRanchoId();
    if(rid){
      try{
        const r=await fetch(`${SB_URL}/rest/v1/animales?rancho_id=eq.${rid}&select=*`,{headers:SB_HEADERS});
        const d=await r.json();
...
function arbolBuscar(q){
  const sug=document.getElementById('arbol-sugerencias');
  if(!sug) return;
  if(!q.trim()){ sug.style.display='none'; return; }

  const animales=typeof DB_ANIMALES!=='undefined'?DB_ANIMALES:[];
  const lista=animales
    .filter(a=>(a.arete||'').toLowerCase().includes(q.toLowerCase())||(a.nombre||'').toLowerCase().includes(q.toLowerCase()))
    .slice(0,8);

  if(!lista.length){ sug.style.display='none'; return; }

  // Posicionar sugerencias debajo del input
  const input=document.getElementById('arbol-buscar');
  if(input){
    ...
    sug.style.borderRadius='10px';
    sug.style.boxShadow='0 8px 24px rgba(0,0,0,.15)';
    sug.style.maxHeight='220px';
    sug.style.overflowY='auto';
  }

  sug.style.display='block';
  sug.innerHTML=lista.map(a=>`
    <div onclick="arbolSeleccionar('${a.id}')"
      style="padding:10px 16px;cursor:pointer;border-bottom:1px solid #F0F4FA;display:flex;align-items:center;gap:10px;"
      onmouseover="this.style.background='#F8FAFF'" onmouseout="this.style.background='#fff'">
      <span style="font-size:18px;">🐄</span>
      <div>
        <div style="font-weight:700;color:#0D2B6B;font-size:13px;">${a.arete||''}${a.nombre?' — '+a.nombre:''}</div>
        <div style="font-size:11px;color:#8FA3BF;">${a.raza||''} · ${a.sexo||''} · ${a.estado||''}</div>
      </div>
    </div>`).join('');

  // Cerrar al clic fuera
  setTimeout(()=>{
    const cerrar=e=>{ if(!sug.contains(e.target)&&e.target.id!=='arbol-buscar'){ sug.style.display='none'; document.removeEventListener('click',cerrar); } };
    document.addEventListener('click',cerrar);
  },100);
}

function arbolSeleccionar(id){
  const sug=document.getElementById('arbol-sugerencias');
  ...
  if(!animal) return;
  if(input) input.value=`${animal.arete||''}${animal.nombre?' — '+animal.nombre:''}`;

  _arbolAnimalSel=animal;
  _arbolRenderizar(animal);
}

function _arbolRenderizar(animal){
  const cont=document.getElementById('arbol-cont');
  if(!cont) return;

  const animales=typeof DB_ANIMALES!=='undefined'?DB_ANIMALES:[];

  /* Buscar parientes */
  const madre  =animales.find(a=>a.arete===animal.madre);
  const padre  =animales.find(a=>a.arete===animal.padre);
  const hijos  =animales.filter(a=>a.madre===animal.arete||a.padre===animal.arete);
  const abMat  =madre?animales.find(a=>a.arete===madre.madre):null;
  const abPatMat=madre?animales.find(a=>a.arete===madre.padre):null;
  const abPat  =padre?animales.find(a=>a.arete===padre.madre):null;
  const abPatPat=padre?animales.find(a=>a.arete===padre.padre):null;

  const card=(a,size='normal',label='')=>{
    if(!a) return `<div style="background:#F3F4F6;border:2px dashed #E0E8F4;border-radius:12px;padding:${size==='sm'?'8px 12px':'12px 16px'};text-align:center;color:#C0C8D8;font-size:${size==='sm'?'11px':'12px'};">
      ${label?`<div style="font-size:10px;color:#C0C8D8;margin-bottom:2px;">${label}</div>`:''}
      Sin registro
    </div>`;
    const COLORES={'Activo':'#22C55E','Gestante':'#9333EA','En tratamiento':'#F0A500','Muerto':'#E24B4A','Vendido':'#6B7280'};
    const col=COLORES[a.estado]||'#2E7DD6';
    const inicial=(a.raza||a.nombre||'?').charAt(0).toUpperCase();
    return `<div onclick="arbolSeleccionar('${a.id}')" style="background:#fff;border:2px solid ${col}30;border-radius:12px;padding:${size==='sm'?'8px 12px':'12px 16px'};cursor:pointer;transition:all .2s;text-align:center;"
      onmouseover="this.style.borderColor='${col}';this.style.boxShadow='0 4px 12px ${col}30'"
      onmouseout="this.style.borderColor='${col}30';this.style.boxShadow='none'">
      ${label?`<div style="font-size:10px;color:#8FA3BF;margin-bottom:4px;font-weight:600;">${label}</div>`:''}
      <div style="width:${size==='sm'?32:40}px;height:${size==='sm'?32:40}px;border-radius:50%;background:${col}20;display:flex;align-items:center;justify-content:center;margin:0 auto 6px;font-family:'Montserrat',sans-serif;font-weight:800;color:${col};font-size:${size==='sm'?'13px':'16px'}">${inicial}</div>
      <div style="font-family:'Montserrat',sans-serif;font-weight:700;color:#0D2B6B;font-size:${size==='sm'?'11px':'13px'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${a.arete||'—'}</div>
      ${a.nombre?`<div style="font-size:10px;color:#8FA3BF;">${a.nombre}</div>`:''}
      <div style="font-size:10px;color:#8FA3BF;margin-top:2px;">${a.raza||''}</div>
      <span style="background:${col}20;color:${col};font-size:9px;font-weight:700;padding:2px 7px;border-radius:20px;">${a.estado||'—'}</span>
    </div>`;
  };

  /* Animal central con más info */
  const COLORES={'Activo':'#22C55E','Gestante':'#9333EA','En tratamiento':'#F0A500','Muerto':'#E24B4A','Vendido':'#6B7280'};
  const col=COLORES[animal.estado]||'#2E7DD6';
  const cardCentral=`
    <div style="background:linear-gradient(135deg,${col}15,${col}05);border:3px solid ${col};border-radius:16px;padding:20px;text-align:center;position:relative;">
      <div style="position:absolute;top:-10px;left:50%;transform:translateX(-50%);background:${col};color:#fff;font-size:10px;font-weight:700;padding:2px 12px;border-radius:10px;">ANIMAL SELECCIONADO</div>
      <div style="width:56px;height:56px;border-radius:50%;background:${col}30;display:flex;align-items:center;justify-content:center;margin:8px auto 10px;font-family:'Montserrat',sans-serif;font-weight:800;color:${col};font-size:24px;">${(animal.raza||animal.nombre||'?').charAt(0).toUpperCase()}</div>
      <div style="font-family:'Montserrat',sans-serif;font-weight:800;color:#0D2B6B;font-size:16px;">${animal.arete||'—'}</div>
      ${animal.nombre?`<div style="font-size:13px;color:#5A6A85;margin:2px 0;">${animal.nombre}</div>`:''}
      <div style="font-size:12px;color:#8FA3BF;">${animal.raza||''} · ${animal.sexo||''}</div>
      ${animal.nacimiento?`<div style="font-size:11px;color:#8FA3BF;">Nac: ${animal.nacimiento}</div>`:''}
      <span style="background:${col};color:#fff;font-size:10px;font-weight:700;padding:3px 10px;border-radius:20px;margin-top:6px;display:inline-block;">${animal.estado||'—'}</span>
    </div>`;

  cont.innerHTML=`
    <!-- ABUELOS -->
    ${(abMat||abPatMat||abPat||abPatPat)?`
    <div style="margin-bottom:4px;">
      <div style="font-size:11px;font-weight:700;color:#8FA3BF;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;text-align:center;">🧬 Abuelos</div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;">
        ${card(abMat,'sm','Abuela materna')}
        ${card(abPatMat,'sm','Abuelo materno')}
        ${card(abPat,'sm','Abuela paterna')}
        ${card(abPatPat,'sm','Abuelo paterno')}
      </div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:0;margin-bottom:0;">
      <div style="text-align:center;padding:4px 0;">
        <div style="width:2px;height:20px;background:#E0E8F4;margin:0 auto;"></div>
      </div>
      <div style="text-align:center;padding:4px 0;">
        <div style="width:2px;height:20px;background:#E0E8F4;margin:0 auto;"></div>
      </div>
    ...
    <!-- PADRES -->
    <div style="margin-bottom:4px;">
      <div style="font-size:11px;font-weight:700;color:#8FA3BF;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;text-align:center;">👨‍👩‍👦 Padres</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
        ${card(madre,'normal','🐄 Madre'+(madre?'':' ('+animal.madre+')'))}
        ${card(padre,'normal','🐂 Padre'+(padre?'':' ('+animal.padre+')'))}
      </div>
    </div>

    <!-- LÍNEA CENTRAL -->
    <div style="text-align:center;padding:8px 0;">
      <div style="width:2px;height:24px;background:#2E7DD6;margin:0 auto;"></div>
    </div>

    <!-- ANIMAL CENTRAL -->
    <div style="margin-bottom:8px;">${cardCentral}</div>

    <!-- LÍNEA A HIJOS -->
    ${hijos.length?`<div style="text-align:center;padding:8px 0;">
      <div style="width:2px;height:24px;background:#22C55E;margin:0 auto;"></div>
    </div>`:''}

    <!-- HIJOS -->
    ${hijos.length?`
    <div>
      <div style="font-size:11px;font-weight:700;color:#8FA3BF;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;text-align:center;">🐣 Descendencia (${hijos.length})</div>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:8px;">
        ${hijos.map(h=>card(h,'sm',h.madre===animal.arete?'Cría (madre)':'Cría (padre)')).join('')}
      </div>
    </div>`:`
...
async function cargarFinanzas(){
  const ranchoId=await _asegurarRanchoId();
  if(!ranchoId){ console.warn('[Finanzas] Sin ranchoId'); return; }

  // Siempre cargar desde Supabase para tener datos frescos
  let gastos=[];
  try{
    const r=await fetch(`${SB_URL}/rest/v1/gastos?rancho_id=eq.${ranchoId}&select=*&order=fecha.desc`,{headers:SB_HEADERS});
    const d=await r.json();
    console.log('[Finanzas] gastos:', r.status, Array.isArray(d)?d.length:'err', d);
    if(Array.isArray(d)) gastos=d;
    DB_GASTOS=gastos;
...
/* ── Stats ── */
function _finActualizarStats(gastos){
...
function _finRenderTabla(gastos){
  gastos=gastos||(typeof DB_GASTOS!=='undefined'?DB_GASTOS:[]);
  const tbody=document.getElementById('fin-tbody');
  if(!tbody) return;
  const esI=g=>g.es_ingreso===true||g.es_ingreso==='true'||g.es_ingreso===1;
  const ultimos=gastos.slice(0,15);
  if(!ultimos.length){
...
function _finRenderCategorias(gastos){
  gastos=gastos||(typeof DB_GASTOS!=='undefined'?DB_GASTOS:[]);
  const esI=g=>g.es_ingreso===true||g.es_ingreso==='true'||g.es_ingreso===1;
