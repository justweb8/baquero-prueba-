...
async function iniciarSesion(){
  ...
  try{
    ...
    if(!authRes.ok) throw new Error(authData.error_description||authData.error||'Credenciales incorrectas');
    ...
    const perfilRes = await fetch(
      `${SB_URL}/rest/v1/perfiles?user_id=eq.${userId}&select=*&limit=1`,
      {headers:SB_HEADERS}
    );
    const perfil = (await perfilRes.json())[0] || {};
    ...
    const rancho = (await ranchoRes.json())[0] || {};
