...
function imInicializar(){
  ...
  if(inp && !inp._imConectado){
    inp.addEventListener('change', e => {
      if(e.target.files[0]) imCargarArchivo(e.target.files[0]);
    });
    inp._imConectado = true;
  }

  if(zona && !zona._imConectado){
    ...
    zona.addEventListener('drop', e => {
      ...
      if(e.dataTransfer.files[0]) imCargarArchivo(e.dataTransfer.files[0]);
    });
    zona._imConectado = true;
  }
}
