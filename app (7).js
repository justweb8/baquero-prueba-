...
// ══ INIT ══
updateOnlineStatus();
const _fl=document.getElementById('fecha-label'); if(_fl) _fl.textContent=new Date().toLocaleDateString('es-PE',{weekday:'long',year:'numeric',month:'long',day:'numeric'});
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if(urlToken) {
    ...
    if(tokenType === 'recovery') {
      window.history.replaceState({}, '', window.location.pathname);
      { const _ld=document.getElementById('app-loading'); if(_ld) _ld.style.display='none'; }
