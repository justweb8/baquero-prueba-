...
async function renderReportes(){
  ...
  const sg=async t=>{
    try{
      const r=await fetch(`${SB_URL}/rest/v1/${t}?rancho_id=eq.${rid}&select=*`,{headers:SB_HEADERS});
      const d=await r.json(); return Array.isArray(d)?d:[];
    }catch(e){ return []; }
  };
  ...
  const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v;};
  set('rep-s-total',  DB_ANIMALES.length);
  set('rep-s-gest',   DB_ANIMALES.filter(a=>a.estado==='Gestante').length);
  set('rep-s-partos', DB_PARTOS.filter(p=>(p.fecha||'').startsWith(anyo+'')).length);
  set('rep-s-insem',  DB_INSEM.length);
  set('rep-s-vac',    DB_SALUD.filter(s=>(s.fecha_aplicacion||'').startsWith(anyo+'')).length);
  set('rep-s-bajas',  DB_ANIMALES.filter(a=>a.estado==='Muerto').length);

  setTimeout(()=>{
    _repRenderRazas();
    _repRenderHato();
    _repRenderInsem();
    _repRenderNacimientos();
    _repRenderSexo();
  }, 200);
}

/* ── 1. Animales por Raza ── */
function _repRenderRazas(){
  const cont=document.getElementById('rep-razas-cont');
  if(!cont||!DB_ANIMALES.length) return;

  const razas={};
  DB_ANIMALES.forEach(a=>{ const r=a.raza||'Sin raza'; razas[r]=(razas[r]||0)+1; });
  const sorted=Object.entries(razas).sort((a,b)=>b[1]-a[1]);
  const max=sorted[0]?.[1]||1;
  const COLORES=['#2E7DD6','#22C55E','#F0A500','#E24B4A','#9333EA','#0891B2','#DC2626'];

  cont.innerHTML=sorted.map(([raza,n],i)=>`
    <div class="rep-raza-item">
      <span class="rep-raza-nombre">${raza}</span>
      <div class="rep-raza-bar-wrap">
        <div class="rep-raza-bar" style="width:${Math.round(n/max*100)}%;background:${COLORES[i%COLORES.length]};"></div>
      </div>
      <span class="rep-raza-num">${n}</span>
      <span class="rep-raza-pct">${Math.round(n/DB_ANIMALES.length*100)}%</span>
    </div>`).join('');
}

/* ── 2. Estado del Hato (donut) ── */
function _repRenderHato(){
  const ley=document.getElementById('rep-hato-ley');
  const canvas=document.getElementById('donut-hato');
  if(!DB_ANIMALES.length) return;

  const estados={};
  DB_ANIMALES.forEach(a=>{ const e=a.estado||'Sin estado'; estados[e]=(estados[e]||0)+1; });
  const sorted=Object.entries(estados).sort((a,b)=>b[1]-a[1]);
  const total=DB_ANIMALES.length;

  const COLORES={'Activo':'#22C55E','Gestante':'#9333EA','En tratamiento':'#F0A500','Muerto':'#E24B4A','Vendido':'#2E7DD6'};
  const vals=sorted.map(([k])=>estados[k]);
  const cols=sorted.map(([k])=>COLORES[k]||'#8FA3BF');

  // Leyenda
  if(ley){
    ley.innerHTML=sorted.map(([estado,n])=>`
      <div class="rep-donut-item">
        <div style="width:10px;height:10px;border-radius:50%;background:${COLORES[estado]||'#8FA3BF'};flex-shrink:0;"></div>
        <span class="rep-donut-lbl">${estado}</span>
        <span class="rep-donut-num">${n}</span>
        <span class="rep-donut-pct">${Math.round(n/total*100)}%</span>
      </div>`).join('');
  }
  ...
  if(canvas&&window.Chart){
    ...
    canvas._chart=new Chart(canvas,{
      type:'doughnut',
      data:{labels:sorted.map(([k])=>k),datasets:[{data:vals,backgroundColor:cols,borderWidth:2,borderColor:'#fff'}]},
      options:{responsive:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>`${ctx.label}: ${ctx.raw} (${Math.round(ctx.raw/total*100)}%)`}}},cutout:'70%'}
    });
  }
}

/* ── 3. Resultado de Inseminaciones ── */
function _repRenderInsem(){
  const cont=document.getElementById('rep-insem-cont');
  if(!cont) return;

  const total=DB_INSEM.length;
  if(!total){
    cont.innerHTML='<div style="text-align:center;color:#8FA3BF;font-size:12px;padding:20px;">Sin inseminaciones registradas</div>';
    return;
  }

  const resultados={};
  DB_INSEM.forEach(i=>{ const r=i.resultado||'Pendiente'; resultados[r]=(resultados[r]||0)+1; });
  const COLORES={'Preñada':'#22C55E','Pendiente':'#F0A500','Fallida':'#E24B4A','Vacía':'#8FA3BF'};

  cont.innerHTML=Object.entries(resultados).sort((a,b)=>b[1]-a[1]).map(([res,n])=>`
    <div class="rep-insem-item">
      <span class="rep-insem-lbl">${res}</span>
      <div class="rep-insem-bar-wrap">
        <div class="rep-insem-bar" style="width:${Math.round(n/total*100)}%;background:${COLORES[res]||'#2E7DD6'};"></div>
      </div>
      <span class="rep-insem-num">${n}</span>
      <span class="rep-insem-pct">${Math.round(n/total*100)}%</span>
    </div>`).join('');

  // Canvas de insem si existe
  const canvas=document.getElementById('chart-partos');
  if(canvas&&window.Chart){
    if(canvas._chart) canvas._chart.destroy();
    const entries=Object.entries(resultados);
    canvas._chart=new Chart(canvas,{
      type:'bar',
      data:{labels:entries.map(([k])=>k),datasets:[{data:entries.map(([,v])=>v),backgroundColor:entries.map(([k])=>COLORES[k]||'#2E7DD6'),borderRadius:4}]},
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,ticks:{stepSize:1}},x:{grid:{display:false}}}}
    });
  }
}

/* ── 4. Nacimientos por Mes ── */
function _repRenderNacimientos(){
  const canvas=document.getElementById('chart-nac');
  if(!canvas||!window.Chart) return;

  const anyo=new Date().getFullYear();
  const MESES=['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'];
  const porMes=Array(12).fill(0);
  DB_PARTOS.filter(p=>(p.fecha||'').startsWith(anyo+'')).forEach(p=>{
    const mes=parseInt((p.fecha||'').split('-')[1])-1;
    if(mes>=0&&mes<12) porMes[mes]++;
  });

  if(canvas._chart) canvas._chart.destroy();
  canvas._chart=new Chart(canvas,{
    type:'bar',
    data:{labels:MESES,datasets:[{label:'Nacimientos',data:porMes,backgroundColor:'#2E7DD680',borderColor:'#2E7DD6',borderWidth:2,borderRadius:4}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,ticks:{stepSize:1}},x:{grid:{display:false}}}}
  });
}

/* ── 5. Distribución por Sexo ── */
function _repRenderSexo(){
  const ley=document.getElementById('rep-sexo-ley');
  const canvas=document.getElementById('donut-sexo');
  if(!DB_ANIMALES.length) return;

  const hembras=DB_ANIMALES.filter(a=>a.sexo==='Hembra').length;
  const machos=DB_ANIMALES.filter(a=>a.sexo==='Macho').length;
  const total=DB_ANIMALES.length;

  if(ley){
    ley.innerHTML=`
      <div class="rep-donut-item"><div style="width:10px;height:10px;border-radius:50%;background:#2E7DD6;flex-shrink:0;"></div><span class="rep-donut-lbl">Hembras</span><span class="rep-donut-num">${hembras}</span><span class="rep-donut-pct">${Math.round(hembras/total*100)}%</span></div>
      <div class="rep-donut-item"><div style="width:10px;height:10px;border-radius:50%;background:#22C55E;flex-shrink:0;"></div><span class="rep-donut-lbl">Machos</span><span class="rep-donut-num">${machos}</span><span class="rep-donut-pct">${Math.round(machos/total*100)}%</span></div>`;
  }
  ...
  if(canvas&&window.Chart){
    ...
    canvas._chart=new Chart(canvas,{
      type:'doughnut',
      data:{labels:['Hembras','Machos'],datasets:[{data:[hembras,machos],backgroundColor:['#2E7DD6','#22C55E'],borderWidth:2,borderColor:'#fff'}]},
...
/* ══ MÓDULOS FALTANTES — Supabase ══ */

async function cargarGastos(){
  const rid=await _asegurarRanchoId(); if(!rid) return;
  try{
    const r=await fetch(`${SB_URL}/rest/v1/gastos?rancho_id=eq.${rid}&select=*&order=fecha.desc`,{headers:SB_HEADERS});
    const d=await r.json(); if(Array.isArray(d)) DB_GASTOS=d;
  }catch(e){}
  if(typeof renderGastos==='function') renderGastos();
  else{
    // Actualizar stats básicos
    const esI=g=>g.es_ingreso===true||g.es_ingreso==='true';
    ...
    const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v;};
    set('gas-s-ing',fmt(ing)); set('gas-s-gas',fmt(gas)); set('gas-s-bal',fmt(ing-gas));
    const tbody=document.getElementById('gas-tbody');
    if(tbody){
      if(!DB_GASTOS.length){tbody.innerHTML='<tr><td colspan="8" style="text-align:center;padding:24px;color:#8FA3BF;">Sin movimientos registrados</td></tr>';return;}
      const MESES=['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'];
      tbody.innerHTML=DB_GASTOS.slice(0,20).map(g=>{
        const ei=esI(g); const monto=parseFloat(g.monto||0);
        let dia='—',mes='';
        if(g.fecha){const d=new Date(g.fecha+'T12:00:00');dia=d.getDate();mes=MESES[d.getMonth()];}
        return `<tr>
          <td style="padding:10px 8px;font-size:12px;">${dia} ${mes}</td>
          <td style="padding:10px 8px;font-size:12px;font-weight:600;color:#0D2B6B;">${g.descripcion||g.tipo||'—'}</td>
          <td style="padding:10px 8px;font-size:11px;color:#5A6A85;">${g.tipo||'—'}</td>
          <td style="padding:10px 8px;font-size:11px;color:#5A6A85;">—</td>
          <td style="padding:10px 8px;font-size:11px;color:#5A6A85;">${g.animal||'—'}</td>
          <td style="padding:10px 8px;text-align:right;font-weight:700;color:${ei?'#22C55E':'#E24B4A'};">${ei?'+':'-'}${fmt(monto)}</td>
          <td style="padding:10px 8px;"><span style="background:${ei?'#ECFDF5':'#FFF0F0'};color:${ei?'#22C55E':'#E24B4A'};padding:2px 8px;border-radius:10px;font-size:10px;font-weight:700;">${ei?'Ingreso':'Gasto'}</span></td>
          <td style="padding:10px 8px;">
            <button onclick="editarGasto('${g.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;">✏️</button>
            <button onclick="eliminarGasto('${g.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;">🗑</button>
          </td>
        </tr>`; }).join('');
    }
    const info=document.getElementById('gas-pag-info');
    if(info) info.textContent=`Mostrando ${Math.min(20,DB_GASTOS.length)} de ${DB_GASTOS.length} registros`;
  }
}
...
async function cargarInventario(){
  const rid=await _asegurarRanchoId(); if(!rid) return;
  try{
    const r=await fetch(`${SB_URL}/rest/v1/inventario?rancho_id=eq.${rid}&select=*&order=nombre.asc`,{headers:SB_HEADERS});
    const d=await r.json(); if(Array.isArray(d)) DB_INVENTARIO=d;
  }catch(e){}
  if(typeof renderInventario==='function') renderInventario();
  else{
    const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v;};
    set('inv-s-total', DB_INVENTARIO.length);
    set('inv-s-bajo',  DB_INVENTARIO.filter(i=>parseFloat(i.stock||0)<=parseFloat(i.minimo||0)&&parseFloat(i.stock||0)>0).length);
    set('inv-s-agotado',DB_INVENTARIO.filter(i=>parseFloat(i.stock||0)===0).length);
    const val=DB_INVENTARIO.reduce((s,i)=>s+parseFloat(i.stock||0)*parseFloat(i.precio||0),0);
    set('inv-s-valor','S/ '+val.toLocaleString('es-PE',{minimumFractionDigits:2}));
    const tbody=document.getElementById('inv-tbody');
    if(tbody){
      if(!DB_INVENTARIO.length){tbody.innerHTML='<tr><td colspan="10" style="text-align:center;padding:24px;color:#8FA3BF;">Sin productos en inventario</td></tr>';return;}
      tbody.innerHTML=DB_INVENTARIO.map(i=>{
        const bajo=parseFloat(i.stock||0)<=parseFloat(i.minimo||0);
        const col=parseFloat(i.stock||0)===0?'#E24B4A':bajo?'#F0A500':'#22C55E';
        return `<tr>
          <td style="padding:10px 8px;font-size:12px;font-weight:600;color:#0D2B6B;">${i.nombre||'—'}</td>
          <td style="padding:10px 8px;font-size:11px;color:#5A6A85;">${i.categoria||'—'}</td>
          <td style="padding:10px 8px;font-size:11px;color:#5A6A85;">${i.unidad||'—'}</td>
          <td style="padding:10px 8px;font-weight:700;color:${col};">${i.stock||0}</td>
          <td style="padding:10px 8px;font-size:11px;color:#5A6A85;">${i.minimo||0}</td>
          <td style="padding:10px 8px;font-size:11px;color:#5A6A85;">${i.precio?'S/ '+parseFloat(i.precio).toFixed(2):'—'}</td>
          <td style="padding:10px 8px;font-size:11px;color:#5A6A85;">${i.proveedor||'—'}</td>
          <td style="padding:10px 8px;font-size:11px;color:#5A6A85;">${i.vencimiento||'—'}</td>
          <td style="padding:10px 8px;"><span style="background:${col}20;color:${col};padding:2px 8px;border-radius:10px;font-size:10px;font-weight:700;">${parseFloat(i.stock||0)===0?'Agotado':bajo?'Stock bajo':'OK'}</span></td>
          <td style="padding:10px 8px;">
            <button onclick="editarInventario('${i.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;">✏️</button>
            <button onclick="eliminarInventario('${i.id}')" style="background:none;border:none;cursor:pointer;font-size:14px;">🗑</button>
          </td>
        </tr>`; }).join('');
    }
  }
...
async function cargarProveedores(){
  const rid=await _asegurarRanchoId(); if(!rid) return;
  try{
    const r=await fetch(`${SB_URL}/rest/v1/proveedores?rancho_id=eq.${rid}&select=*&order=nombre.asc`,{headers:SB_HEADERS});
    const d=await r.json(); if(Array.isArray(d)) DB_PROVEEDORES=d;
  }catch(e){}
  if(typeof renderProveedores==='function') renderProveedores();
  else{
    const lista=document.getElementById('prov-lista');
    if(!lista) return;
    if(!DB_PROVEEDORES.length){lista.innerHTML='<div style="text-align:center;padding:40px;color:#8FA3BF;">Sin proveedores registrados</div>';return;}
    lista.innerHTML=DB_PROVEEDORES.map(p=>`
      <div style="background:#fff;border:1px solid #E8EEF8;border-radius:14px;padding:16px;margin-bottom:10px;display:flex;align-items:center;gap:14px;">
        ...
        <div style="flex:1;">
          <div style="font-family:'Montserrat',sans-serif;font-size:14px;font-weight:700;color:#0D2B6B;">${p.nombre||'—'}</div>
          <div style="font-size:11px;color:#8FA3BF;">${p.tipo||''} ${p.telefono?'· '+p.telefono:''} ${p.ciudad?'· '+p.ciudad:''}</div>
        </div>
        <div style="display:flex;gap:6px;">
          <button onclick="editarProveedor('${p.id}')" style="background:#EFF6FF;border:none;border-radius:8px;padding:6px 12px;cursor:pointer;font-size:12px;color:#2E7DD6;">✏️ Editar</button>
        </div>
      </div>`).join('');
  }
}
...
async function cargarBanco(){
  const rid=await _asegurarRanchoId(); if(!rid) return;
  try{
    const r=await fetch(`${SB_URL}/rest/v1/semen_toros?rancho_id=eq.${rid}&select=*&order=created_at.desc`,{headers:SB_HEADERS});
    const d=await r.json(); if(Array.isArray(d)) DB_BANCO=d;
  }catch(e){}
  const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v;};
  set('bk-s-toros',   DB_BANCO.length);
  set('bk-s-activos', DB_BANCO.filter(t=>t['Dep']==='Activo'||t.dep==='Activo').length);
  const razas=new Set(DB_BANCO.map(t=>t['Raza']||t.raza).filter(Boolean));
  set('bk-s-razas', razas.size);
  const dosis=DB_BANCO.reduce((s,t)=>s+parseInt(t['Pajillas']||t.pajillas||0),0);
  set('bk-s-dosis', dosis);
  const grid=document.getElementById('bk-grid');
  if(grid){
    if(!DB_BANCO.length){grid.innerHTML='<div style="text-align:center;padding:40px;color:#8FA3BF;grid-column:1/-1;">Sin toros en el banco genético</div>';return;}
    grid.innerHTML=DB_BANCO.map(t=>`
      <div style="background:#fff;border:1px solid #E8EEF8;border-radius:14px;padding:16px;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;">
          <div style="font-family:'Montserrat',sans-serif;font-size:14px;font-weight:700;color:#0D2B6B;">${t['Nombre']||t.nombre||'—'}</div>
          <span style="background:#EFF6FF;color:#2E7DD6;font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;">${t['Raza']||t.raza||'—'}</span>
        </div>
        <div style="font-size:11px;color:#8FA3BF;">
          ${t['Empresa']?`🏢 ${t['Empresa']}<br>`:''}
          ${t['Pais']?`🌍 ${t['Pais']}<br>`:''}
          🧬 Pajillas: <b style="color:#0D2B6B;">${t['Pajillas']||t.pajillas||0}</b>
          ${t['Precio']?` · S/ ${t['Precio']}`:''}
        </div>
        <div style="margin-top:10px;display:flex;gap:6px;">
          <button onclick="bkModal(true,'${t.id}')" style="background:#EFF6FF;border:none;border-radius:8px;padding:5px 10px;cursor:pointer;font-size:11px;color:#2E7DD6;">✏️ Editar</button>
          <button onclick="eliminarToro('${t.id}')" style="background:#FFF0F0;border:none;border-radius:8px;padding:5px 10px;cursor:pointer;font-size:11px;color:#E24B4A;">🗑</button>
        </div>
      </div>`).join('');
  }
}
...
async function cargarHistorial(){
  const rid=await _asegurarRanchoId(); if(!rid) return;
  try{
    const r=await fetch(`${SB_URL}/rest/v1/historial_docs?rancho_id=eq.${rid}&select=*&order=created_at.desc&limit=50`,{headers:SB_HEADERS});
    const d=await r.json(); if(Array.isArray(d)) DB_DOCS=d;
  }catch(e){}
  if(typeof renderHistorialDocs==='function') renderHistorialDocs();
  else{
    const grid=document.getElementById('his-grid');
    if(!grid) return;
    if(!DB_DOCS.length){grid.innerHTML='<div style="text-align:center;padding:40px;color:#8FA3BF;">Sin registros en el historial</div>';return;}
    const MESES=['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'];
    grid.innerHTML=DB_DOCS.map(d=>{
      const f=new Date(d.created_at||d.fecha||'');
      const dia=isNaN(f)?'—':f.getDate()+' '+MESES[f.getMonth()];
      return `<div style="background:#fff;border:1px solid #E8EEF8;border-radius:12px;padding:14px;display:flex;gap:12px;align-items:flex-start;">
        ...
        <div>
          <div style="font-size:12px;font-weight:600;color:#0D2B6B;">${d.tipo||'Registro'}</div>
          <div style="font-size:11px;color:#5A6A85;">${d.descripcion||'—'}</div>
          <div style="font-size:10px;color:#8FA3BF;margin-top:4px;">${dia}</div>
        </div>
      </div>`;}).join('');
    const info=document.getElementById('his-pag-info');
    if(info) info.textContent=`${DB_DOCS.length} registros`;
  }
}
...
async function cargarDocumentos(){
  const rid=await _asegurarRanchoId(); if(!rid) return;
  try{
    const r=await fetch(`${SB_URL}/rest/v1/historial_docs?rancho_id=eq.${rid}&select=*&order=created_at.desc`,{headers:SB_HEADERS});
    const d=await r.json(); if(Array.isArray(d)) DB_DOCS=d;
  }catch(e){}
  const tbody=document.getElementById('dc-tbody');
  if(tbody){
    if(!DB_DOCS.length){tbody.innerHTML='<tr><td colspan="7" style="text-align:center;padding:24px;color:#8FA3BF;">Sin documentos</td></tr>';return;}
    tbody.innerHTML=DB_DOCS.map(d=>`<tr>
      <td style="padding:10px 8px;font-size:12px;">${d.num_doc||'—'}</td>
      <td style="padding:10px 8px;font-size:12px;">${d.tipo||'—'}</td>
      <td style="padding:10px 8px;font-size:12px;">${d.descripcion||'—'}</td>
      <td style="padding:10px 8px;font-size:11px;color:#5A6A85;">${d.fecha||'—'}</td>
      <td style="padding:10px 8px;font-size:11px;color:#5A6A85;">${d.hora||'—'}</td>
...
async function cargarPerfil(){
  if(typeof cargarPerfilDesdeDB==='function') cargarPerfilDesdeDB();
  else if(typeof cargarConfigPerfil==='function') cargarConfigPerfil();
}

async function cargarSuscripcion(){
  if(typeof renderSuscripcion==='function') renderSuscripcion();
}
...
function exInicializar(){
  const actualizar=async()=>{
    ...
    const rid=await _asegurarRanchoId(); if(!rid) return;
    const sg=async t=>{try{const r=await fetch(`${SB_URL}/rest/v1/${t}?rancho_id=eq.${rid}&select=id`,{headers:SB_HEADERS});const d=await r.json();return Array.isArray(d)?d.length:0;}catch(e){return 0;}};
    const [cA,cS,cG,cI]=await Promise.all([
      DB_ANIMALES.length||sg('animales'),DB_SALUD.length||sg('salud'),
      DB_GASTOS.length||sg('gastos'),DB_INVENTARIO.length||sg('inventario')
    ]);
    const nums=document.querySelectorAll('#sec-exportar .ex-stat-num');
    [cA,cS,cG,cI].forEach((v,i)=>{if(nums[i])nums[i].textContent=v;});
  };
  actualizar();setTimeout(actualizar,400);
  setTimeout(()=>{window.exDescargar=exDescargar;},300);
}
...
function imInicializar(){
  ...
  setTimeout(()=>{
    window.imDescargarPlantilla=function(){
      ...
      const ej=[['A-001','Lolita','Brahman','Hembra','2023-01-15','250','Activo','A-000','—',''],['A-002','Toro1','Angus','Macho','2022-06-20','400','Activo','—','—','']];
      const ws=XLSX.utils.aoa_to_sheet([cols,...ej]);
      const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'Animales');
      XLSX.writeFile(wb,'plantilla_animales.xlsx');
      imActualizarPasos(2);toast('📥 Plantilla descargada');
    };
    window.imSeleccionarArchivo=function(input){
      ...
      if(btnEl){btnEl.removeAttribute('disabled');btnEl.classList.add('ready');btnEl.style.cursor='pointer';btnEl.style.background='#2E7DD6';}
      imActualizarPasos(3);imCargarArchivo(file);
    };
    window.imQuitarArchivo=function(){
      ...
      const dropEl=document.getElementById('im-drop-zone');
      const btnEl=document.getElementById('im-btn-validar');
      if(inp)inp.value='';if(prevEl)prevEl.classList.remove('show');
      if(dropEl)dropEl.style.display='flex';
      if(btnEl){btnEl.setAttribute('disabled','');btnEl.classList.remove('ready');btnEl.style.cursor='not-allowed';btnEl.style.background='';}
      _imArchivo=null;_imDatosPreview=[];
      const prev=document.getElementById('im-preview');
      if(prev){prev.innerHTML='';prev.style.display='none';}
      imActualizarPasos(1);toast('🗑 Archivo eliminado');
    };
    window.imValidar=function(){imActualizarPasos(4);imValidarReal();};
  },300);
}
