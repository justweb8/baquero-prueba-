...
async function imCargarArchivo(file){
  ...
  try{
    ...
    // Activar botón con clase ready (así lo maneja el CSS original)
...
function imQuitarArchivo(){
  ...
  const btnVal = document.getElementById('im-btn-validar');
  if(btnVal){ btnVal.classList.remove('ready'); btnVal.style.cursor='not-allowed'; }
...
async function imValidar(){
  ...
  let tipo = 'animales';
  if(headers.includes('arete') || headers.includes('raza')) tipo='animales';
  else if(headers.includes('animal') && headers.includes('tipo')) tipo='salud';
  else if(headers.includes('monto') || headers.includes('es_ingreso')) tipo='gastos';

  const btn = document.querySelector('#sec-importar .im-btn-importar, #sec-importar button[onclick="imValidar()"]');
  ...
  for(const fila of filas){
    ...
    if(tipo==='animales'){
      if(!obj.arete){ err++; errores.push('Fila sin arete'); continue; }
    ...
    try{
      ...
      if(res.ok) ok++;
      else { err++; const e=await res.json(); errores.push(e.message||'Error'); }
    }catch(e){ err++; }
  }
  ...
  // Mostrar resultado
  const prev=document.getElementById('im-preview');
  if(prev) prev.innerHTML+=`<div style="margin-top:12px;padding:12px;border-radius:8px;background:${err?'#FFF3CD':'#D1FAE5'};color:${err?'#92400E':'#065F46'};font-size:13px;font-weight:600;">${msg}</div>`;
}
